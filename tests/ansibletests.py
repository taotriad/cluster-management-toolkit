...
from clustermanagementtoolkit.cmttypes import FilePath, ArgumentValidationError, FilePathAuditError

from clustermanagementtoolkit.ansithemeprint import ANSIThemeStr
from clustermanagementtoolkit.ansithemeprint import ansithemeprint, init_ansithemeprint

from clustermanagementtoolkit import ansible_helper
...
def test_get_playbook_path(verbose: bool = False) -> tuple[str, bool]:
    ...
    if result:
        ansithemeprint([ANSIThemeStr("  Note", "note"),
                        ANSIThemeStr(": These testcases only work correctly if the playbooks "
        ...
        testdata: tuple[Any, ...] = (
            ("get_versions.yaml",
             (SYSTEM_ANSIBLE_PLAYBOOK_DIR.joinpath("get_versions.yaml"),
        ...
        for indata, expected_result, expected_exception in testdata:
            try:
                if (tmp := fun(indata)) not in expected_result:
...
def test_populate_playbooks_from_filenames(verbose: bool = False) -> tuple[str, bool]:
    message = ""
    result = True

    fun = ansible_helper.populate_playbooks_from_filenames

    # Add test playbooks dir to cmtconfig
    deep_set(cmtlib.cmtconfig, DictPath("Ansible#local_playbooks"),
             [TEST_PLAYBOOKS_DIR], create_path=True)
    ...
    if result:
        ...
        # (playbook, expected_result, expected_exception)
        testdata: tuple[Any, ...] = (
            ([FilePath("get_versions.yaml")],
             [([ANSIThemeStr("  • ", "separator"),
                ANSIThemeStr("Get package versions", "play")],
               (SYSTEM_ANSIBLE_PLAYBOOK_DIR.joinpath("get_versions.yaml"),
                ANSIBLE_PLAYBOOK_DIR.joinpath("get_versions.yaml")))], None),

            ([FilePath("NONEXISTINGPLAYBOOK.yml")], None, FilePathAuditError),

            ([FilePath("hostname.yml")],
             [([ANSIThemeStr("  • ", "separator"),
               ANSIThemeStr("Hostname test playbook; local, lacks summary", "play")],
               (TEST_PLAYBOOKS_DIR.joinpath("hostname.yml"),))], None),

            ([FilePath(".hostname.yml")], None, FilePathAuditError),

            ([FilePath("uptime.yaml")],
             [([ANSIThemeStr("  • ", "separator"),
                ANSIThemeStr("Running “", "play"),
                ANSIThemeStr("uptime", "programname"),
                ANSIThemeStr("“", "play")],
               (TEST_PLAYBOOKS_DIR.joinpath("uptime.yaml"),))], None),
            ([FilePath("~uptime.yaml")], [], None),
            ([FilePath("uptime.json")], None, ValueError),
            ([FilePath("uptime.yaml").joinpath("NonExistingFile.yaml")],
             None, FilePathAuditError),
            ([""], None, ValueError),
            (None, None, TypeError),
            (1, None, TypeError),
        ...
        for indata, expected_result, expected_exception in testdata:
            try:
                ...
                for i, entry in enumerate(tmp):
                    themearray, path = entry
                    expected_themearray, expected_paths = expected_result[i]
                    if themearray != expected_themearray or path not in expected_paths:
                        message = f"{fun.__name__}() did not yield expected result:\n" \
                                  f"          input: {indata}\n" \
                                  f"         output: {tmp}\n" \
                                  f"       expected: {expected_result}"
...
def test_ansible_print_action_summary(verbose: bool = False) -> tuple[str, bool]:
    ...
    if result:
        ...
        # (playbook, expected_result, expected_exception)
        testdata: tuple[Any, ...] = (
            (ansible_helper.populate_playbooks_from_filenames([
                FilePath("get_versions.yaml")]), None),
            (ansible_helper.populate_playbooks_from_filenames([
                FilePath("uptime.yaml")]), None),
            ([], ValueError),
            ([1], TypeError),
...
def test_ansible_get_inventory_dict(verbose: bool = False) -> tuple[str, bool]:
    ...
    if result:
        ...
        testdata: tuple[Any, ...] = (
            ("clustermanagementtoolkit.ansible_helper.ANSIBLE_INVENTORY",
             "/doesnotexist", None,
             {"all": {"hosts": {}, "vars": {}}}),
            ("clustermanagementtoolkit.ansible_helper.secure_read_yaml",
             None, {}, {"all": {"hosts": {}, "vars": {}}}),
            ("clustermanagementtoolkit.ansible_helper.secure_read_yaml", None,
        ...
        try:
            for indata in testdata:
                ...
                if mock_value:
                    ...
                    with mock.patch("clustermanagementtoolkit.ansible_helper.Path.is_file",
                                    return_value=True), \
...
def test_ansible_get_inventory_pretty(verbose: bool = False) -> tuple[str, bool]:
    ...
    if result:
        ...
        with mock.patch("clustermanagementtoolkit.ansible_helper.ANSIBLE_INVENTORY",
                        "/doesnotexist"):
    ...
    if result:
        ...
        for indata in testdata:
            ...
            try:
                with mock.patch("clustermanagementtoolkit.ansible_helper.Path.is_file",
                                return_value=True), \
                        mock.patch("clustermanagementtoolkit.ansible_helper.secure_read_yaml",
                ...
                try:
                    if override_ansible_configuration:
                        deep_set(ansible_helper.ansible_configuration,
                                 DictPath("ansible_user"), "foo", create_path=True)
                        deep_set(ansible_helper.ansible_configuration,
                                 DictPath("ansible_password"), "bar", create_path=True)
                        deep_set(ansible_helper.ansible_configuration,
                    ...
                    if not (output_d and output_d.items() == expected_content.items()):
                        message = f"{fun.__name__}() did not yield expected result:\n" \
                                  " inventory dict:\n" \
                    ...
                    if expected_exception:
                        ...
                        break
            deep_set(ansible_helper.ansible_configuration,
                     DictPath("ansible_user"), real_ansible_user, create_path=True)
            deep_set(ansible_helper.ansible_configuration,
                     DictPath("ansible_password"), real_ansible_password, create_path=True)
            deep_set(ansible_helper.ansible_configuration,
                     DictPath("disable_strict_host_key_checking"),
                     real_ansible_disable_strict_host_key_checking, create_path=True)
            os.unlink(tf.name)
    return message, result
...
def test_ansible_set_groupvars(verbose: bool = False) -> tuple[str, bool]:
    ...
    if result:
        with tempfile.TemporaryDirectory() as td, tempfile.NamedTemporaryFile(delete=False) as tf:
            ...
            testdata: tuple[Any, ...] = (
                (d1, tf.name, ["foo"], [("bar", 1), ("baz", "text")],
                 {"foo": {"hosts": {}, "vars": {"bar": 1, "baz": "text"}}}, True, None),
                (d2, tf.name, ["foo"], [("bar", 1), ("baz", "text")],
                 {"foo": {"hosts": {}, "vars": {"bar": 1, "baz": "text"}}}, True, None),
                (d1, tf.name, ["foo"], [("bar", 1), ("baz", "text")],
                 {"foo": {"hosts": {}, "vars": {"bar": 1, "baz": "text"}}}, True, None),
                (d1, tf.name, ["bar"], [("meaning", 42)],
                 {"foo": {"hosts": {}, "vars": {}}, "bar": {"vars": {"meaning": 42}}}, True, None),
                (d1, tf.name, ["nonexisting"], [("bar", 1), ("baz", "text")],
                 {}, True, None),
                (d1, f"{td}/TemporaryFile", ["foo", "bar"], [("bar", 1), ("baz", "text")],
                 {"foo": {"hosts": {}, "vars": {"bar": 1, "baz": "text"}},
                  "bar": {"hosts": {}, "vars": {"bar": 1, "baz": "text"}}},
                 None, FilePathAuditError),
                (d1, "", ["foo"], [("bar", 1), ("baz", "text")], None,
                 None, ArgumentValidationError),
                (d1, None, ["foo"], [("bar", 1), ("baz", "text")], None,
                 True, ArgumentValidationError),
            )

            for indata in testdata:
                d, path, groups, values, expected_content, \
                ...
                try:
                    ...
                    if not expected_result or groups is None \
                        ...
                        continue
                    output_d = dict(secure_read_yaml(path, temporary=True))
...
def test_ansible_run_playbook(verbose: bool = False) -> tuple[str, bool]:
    ...
    if result:
        ...
        testdata: tuple[Any, ...] = (
            (get_playbook_path(FilePath("uptime.yaml")), d, False, 0, {"Checking uptime": {
            ...
            (get_playbook_path(FilePath("uptime.yaml")), d, True, 0, {"Checking uptime": {
                "task": "Checking uptime",
                "start_date": None,
                "end_date": None,
                "retval": 0,
...
def test_ansible_ping(verbose: bool = False) -> tuple[str, bool]:
    ...
    if result:
        ansithemeprint([ANSIThemeStr("  Note", "note"),
                        ANSIThemeStr(": This check can potentially be very slow.", "default")])
        ansithemeprint([ANSIThemeStr("  Note", "note"),
                        ANSIThemeStr(": This check will fail in networks "
                                     "that resolve non-existing addresses.\n", "default")])
        ...
        for indata in testdata:
            ...
            try:
                tmp = fun(selection)
                if tmp and len(tmp[0]) > 1 and tmp[0][1].startswith("UNREACHABLE (unknown"):
                    print(f"Warning: ping returned {tmp[0][1]}; "
...
tests: dict[tuple[str, ...], dict[str, Any]] = {
    ...
    ("populate_playbooks_from_filenames()",): {
        "callable": test_populate_playbooks_from_filenames,
        "result": None,
    },
