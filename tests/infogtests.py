...
def test_process_value(verbose: bool = False) -> tuple[str, bool]:
    message = ""
    result = True
    ...
    fun = infogetters.process_value

    if result:
        # Indata format:
        # (value, vtype, kwargs, expected_result, expected_exception)
        testdata: tuple = (
            (None, "str", {}, "", None),
            ("foo\nbar", "str", {}, "foo\\nbar", None),
            ("\\\"foo\\\"", "str", {"replace_quotes": "pretty"}, "“foo“", None),
            ("\\\"foo\\\"", "str", {"replace_quotes": "same"}, "\"foo\"", None),
            (1, "str", {}, 1, None),
            (1.0, "float", {}, "1.0", None),
            (1, "int", {}, "1", None),
            (True, "bool", {}, "True", None),
            ((1, 2, 3), "int", {"action": "sum"}, 6, None),
            ([1, 2, 3], "int", {"action": "sum"}, 6, None),
            ((1, 2, 3), "int", {}, ["1", "2", "3"], None),
            ("3", "cpu_usage", {}, "3000.0", None),
            (["3", "5"], "cpu_usage", {"action": "sum"}, "8000.0", None),
            ("3", "cpu_usage_round", {}, "3", None),
            ("15985472Ki", "mem_usage", {}, "15.2GiB", None),
            (["15985472Ki", "15909864Ki"], "mem_usage", {"action": "sum"}, "30.4GiB", None),
            (358255783, "disk_usage", {}, "341MiB", None),
            (None, "len", {}, "0", None),
            ("foobar", "len", {}, "6", None),
            ([1, 2], "len", {}, "2", None),
            ...
            (["1970-01-01 02:00:00", "1970-01-02 02:00:00"], "timestamp", {"action": "earliest"},
             datetime.strptime("1970-01-01 02:00:00+00:00", "%Y-%m-%d %H:%M:%S%z"), None),
            (["1970-01-02 02:00:00", "1970-01-01 02:00:00"], "timestamp", {"action": "earliest"},
             datetime.strptime("1970-01-01 02:00:00+00:00", "%Y-%m-%d %H:%M:%S%z"), None),
            (["1970-01-01 02:00:00", "1970-01-02 02:00:00"], "timestamp", {"action": "latest"},
             datetime.strptime("1970-01-02 02:00:00+00:00", "%Y-%m-%d %H:%M:%S%z"), None),
            (["1970-01-02 02:00:00", "1970-01-01 02:00:00"], "timestamp", {"action": "latest"},
             datetime.strptime("1970-01-02 02:00:00+00:00", "%Y-%m-%d %H:%M:%S%z"), None),
            (["1970-01-01 02:00:00", None], "timestamp", {"action": "latest"},
             datetime.strptime("1970-01-01 02:00:00+00:00", "%Y-%m-%d %H:%M:%S%z"), None),
            (["1970-01-01 02:00:00", -1], "timestamp", {"action": "latest"},
             datetime.strptime("1970-01-01 02:00:00+00:00", "%Y-%m-%d %H:%M:%S%z"), None),
            (["answer", 42], ["raw", "raw"], {}, ["answer", 42], None),
            (["answer", 42], [], {}, ["answer", 42], None),
            (["answer", "fortytwo"], ["name", "name"], {}, ["answer", "fortytwo"], None),
            (("Pod", ""), ["kind", "api_group"], {}, ("Pod", ""), None),
            (("Pod", "v1"), ["kind", "api_group"], {}, ("Pod", ""), None),
            (("DaemonSet", "apps"), ["kind", "api_group"], {}, ("DaemonSet", ""), None),
            (("DaemonSet", "apps/v1"), ["kind", "api_group"], {}, ("DaemonSet", ""), None),
            (("Tensorboard", "tensorboard.kubeflow.org/v1"), ["kind", "api_group"], {},
             ("Tensorboard", "tensorboard.kubeflow.org"), None),
            (["answer", "unknown"], ["name", "skip"], {}, ["answer", ""], None),
            (["answer", "unknown"], ["unknown", "skip"], {}, None, ValueError),
            ("foo", "raw", {}, "foo", None),
            (42, "raw", {}, 42, None),
            (":", "separator", {}, ":", None),
        ...
        for value, vtype, kwargs, expected_result, expected_exception in testdata:
            try:
                tmp = fun(value, vtype, **kwargs)
                if tmp != expected_result:
                    message = f"{fun.__name__}() did not yield expected result:\n" \
                              f"            value: {value}\n" \
                              f"            vtype: {vtype}\n" \
                              f"           result: {tmp} (type: {type(tmp)})\n" \
                              f"  expected result: {expected_result} " \
                              f"(type: {type(expected_result)})\n" \
                              "           kwargs:\n" \
...
def test_transform_filter(verbose: bool = False) -> tuple[str, bool]:
    message = ""
    result = True

    fun = infogetters.transform_filter

    if result:
        # Indata format:
        # (transformations, value, expected_result, expected_exception)
        testdata: tuple = (
            ("foo", {}, "foo", None),
            ("foo", {"foo": "bar"}, "bar", None),
            ("foo", {"baz": "bar"}, "foo", None),
        )

        for value, transformations, expected_result, expected_exception in testdata:
            try:
                tmp = fun(value, transformations)
                if tmp != expected_result:
                    message = f"{fun.__name__}() did not yield expected result:\n" \
                              f"            value: {value}\n" \
                              f"           result: {tmp} (type: {type(tmp)})\n" \
                              ...
                              f"(type: {type(expected_result)})\n" \
                              "  transformations:\n" \
                              f"{yaml_dump(transformations, base_indent=17)}"
                    result = False
                    break
            except Exception as e:
                if expected_exception is not None:
...
def test_when_filter(verbose: bool = False) -> tuple[str, bool]:
    message = ""
    result = True

    fun = infogetters.when_filter

    if result:
        # Indata format:
        # (path, item, key, value, expected_result, expected_exception)
        testdata: tuple = (
            (
                # No rules
                {
            ...
            (
                # key and value not set, when rule missing key
                {
                    "when": [
                        {},
                    ],
            ...
            (
                # Matching key, condition not met
                {
                    "when": [
                        {
                            "key": "type",
            ...
            (
                # notstartswith, condition not met
                {
                    "when": [
                        {
                            "key": "type",
            ...
            (
                # notdicthaskey, condition not met
                {
                    "when": [
                        {
                            "key": "type",
            ...
            (
                # key_startswith, condition not met
                {
                    "when": [
                        {
                            "key_startswith": "foo",
            ...
            (
                # key_notstartswith, condition met
                {
                    "when": [
                        {
                            "key_notstartswith": "foo",
                        },
                    ],
                },
                None,
            ...
            (
                # key_notstartswith, condition not met
                {
                    "when": [
                        {
                            "key_notstartswith": "foo",
            ...
            (
                # key_endswith, condition not met
                {
                    "when": [
                        {
                            "key_endswith": "foo",
            ...
            (
                # key_notendswith, condition not met
                {
                    "when": [
                        {
                            "key_notendswith": "foo",
        ...
        for when_path, item, key, value, expected_result, expected_exception in testdata:
            try:
                tmp = fun(when_path, item, key, value)
                if tmp != expected_result:
                    message = f"{fun.__name__}() did not yield expected result:\n" \
                              f"              key: {key}\n" \
                              f"            value: {value}\n" \
                              f"      when_filter: {deep_get(when_path, DictPath('when'))}\n" \
                              f"             item: {item}\n" \
                              f"           result: {tmp}\n" \
                              f"  expected result: {expected_result}"
                    result = False
                ...
                [
                    ...
                    },
                    {
                        ...
                        "ref": {
                            ...
                            "fully_decoded_value": "A test string with UTF-⑧\n",
                            "vtype": "base64-utf-8",
                            "vlen": 36
                        },
                        "decoded_value": "QSB0ZXN0IHN0cmluZyB3aXRoIFVURi3ikacK",
                ...
                if len(tmp) != len(expected_result):
                    ...
                    for i, item in enumerate(tmp):
                        for key, value in expected_result[i].items():
                            result_value = deep_get(item, DictPath(key))
            ...
            try:
                ...
                for i, item in enumerate(expected_result):
                    for key, value in item.items():
                        result = deep_get(tmp[i], DictPath(key))
                        if len(tmp) != len(expected_result):
                            message = f"{fun.__name__}() did not yield expected result:\n" \
                                      f"   expand_subsets: {expand_subsets}\n" \
                                      f"           result: {key}: {result}\n" \
...
tests: dict[tuple[str, ...], dict[str, Any]] = {
    ("process_value()",): {
        "callable": test_process_value,
        "result": None,
    },
    ("transform_filter()",): {
        "callable": test_transform_filter,
        "result": None,
    },
    ("when_filter()",): {
