...
def test_set_logger(verbose: bool = False) -> tuple[str, bool]:
    message = ""
    result = True

    fun = cmtlog.set_logger

    if result:
        try:
            with tempfile.TemporaryDirectory() as td:
                tempdir = FilePath(td)
                with mock.patch("clustermanagementtoolkit.cmtlog.cmtpaths.CMT_LOGS_DIR", tempdir):
                    fun("cmtlogtessts", LogLevel.INFO)
        except Exception as e:
            message = f"{fun.__name__}() did not yield expected result:\n" \
                      f"      exception: {type(e)}\n"
            result = False
...
def test_set_level(verbose: bool = False) -> tuple[str, bool]:
    message = ""
    result = True

    fun = cmtlog.set_level

    if result:
        # Indata format:
        # (loglevel, expected_result, expected_exception)
        testdata: tuple[Any, ...] = (
            (LogLevel.DEBUG, logging.DEBUG, None),
            (LogLevel.INFO, logging.INFO, None),
            (LogLevel.WARNING, logging.WARNING, None),
            (LogLevel.ERR, logging.ERROR, None),
            (LogLevel.CRIT, logging.CRITICAL, None),
            (None, None, ValueError),
        )

        # First test whether None-check works properly
        with mock.patch("clustermanagementtoolkit.cmtlog.logger", None):
            try:
                fun(LogLevel.DEBUG)
                tmp = cmtlog.logger.getEffectiveLevel()
                message = f"{fun.__name__}() did not yield expected result:\n" \
                          f"          input: {repr(LogLevel)} (logger not initialized)\n" \
                          f"         output: {tmp}\n" \
        ...
        for loglevel, expected_result, expected_exception in testdata:
            try:
                fun(loglevel)
                tmp = cmtlog.logger.getEffectiveLevel()
                if tmp != expected_result:
                    message = f"{fun.__name__}() did not yield expected result:\n" \
                              f"          input: {repr(LogLevel)}\n" \
                              f"         output: {tmp}\n" \
                              f"       expected: {expected_result}"
                    result = False
                    break
...
def test_log(verbose: bool = False) -> tuple[str, bool]:
    message = ""
    result = True
    ...
    if result:
        # Indata format:
        # (loglevel, msg, messages, expected_result, expected_exception)
        testdata: tuple[Any, ...] = (
            (LogLevel.DEBUG, "Test message", None, None, None),
            (LogLevel.INFO, "Test message", None, None, None),
            (LogLevel.WARNING, "Test message", None, None, None),
            (LogLevel.ERR, "Test message", None, None, None),
            (LogLevel.CRIT, "Test message", None, None, None),
            (LogLevel.INFO, None, ["Foo", " bar", " baz"], None, None),
            (LogLevel.INFO, None, [[ANSIThemeStr("Test message", "info")]], None, None),
            (LogLevel.INFO, None, None, None, None),
            (None, None, "Test message", None, ValueError),
        )
        ...
        for loglevel, msg, messages, expected_result, expected_exception in testdata:
            try:
                fun(loglevel, msg=msg, messages=messages)
            except Exception as e:
                if expected_exception is not None:
                    if isinstance(e, expected_exception):
                        pass
...
tests: dict[tuple[str, ...], dict[str, Any]] = {
    ...
    ("set_logger()",): {
        "callable": test_set_logger,
        "result": None,
    },
    ("set_level()",): {
        "callable": test_set_level,
        "result": None,
    },
    ("log()",): {
        "callable": test_log,
