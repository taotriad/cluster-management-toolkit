...
from clustermanagementtoolkit.cmttypes import FilePath, Retval, LogLevel
...
def signal_handler(sig: int, frame: FrameType | None) -> None:
    """
    Signal handler for keypresses.

        Parameters:
            sig (int): The signal to handle [unused]
            frame (FrameType | None): The frame [unused]
    """
    del sig
    del frame
...
defaultlistaction: str = ""  # pylint: disable=invalid-name
defaultpath: tuple[str, str] = ("", "")

kh: KubernetesHelper = None  # type: ignore # pylint: disable=invalid-name
kh_cache: KubernetesResourceCache = None  # type: ignore # pylint: disable=invalid-name
...
def gather_cluster_info(**kwargs: Any) -> None:
    ...
    _control_plane_node, control_plane_name = get_control_planes()[0]
...
def format_timestamp(timestamp: datetime,
    ...
    if timestamp is None:
        ...
        array = generators.format_numerical_with_units(ftimestamp,
                                                       selected=False, ftype="timestamp")
...
def update_field_widths(field_dict: dict, field_names: list[str], objects: list[dict]) -> int:
    ...
    then return the total line length.
        ...
        Parameters:
            ...
            objects ([dict]): A dict with the data for all fields in the field_names list
        Returns:
...
def get_image_tuple(image: str) -> tuple[str, str]:
    ...
    """
    tmp: re.Match[str] | None = re.match(r"^(.*):(.*)", image)
    if tmp is not None:
        image_name = f"{tmp[1]}"
...
def get_control_planes() -> list[tuple[dict[str, Any], str]]:
    """
    Returns name and object for all control planes.

        Returns:
            ([(dict[str, Any], str)]): A list of control planes
                (dict[str, Any]): The object for the control plane
    ...
    for obj in vlist:
        ...
        if "node-role.kubernetes.io/control-plane" in labels \
            ...
            control_planes.append((copy.deepcopy(obj), deep_get(obj, DictPath("metadata#name"))))
    ...
    if not control_planes and len(vlist) == 1:
        control_planes.append((copy.deepcopy(vlist[0]),
                               deep_get(vlist[0], DictPath("metadata#name"))))

    k8s_distro, _status = kh.identify_k8s_distro()

    if not control_planes:
        if k8s_distro != "kamaji":
...
def generate_list_row(uip: UIProps, data: dict, field_dict: dict,
    ...
    Generate a list for the listpad.
        ...
        Parameters:
            uip (UIProps): A reference to the UI Properties object
            data (dict): The data to generate a list row from
...
def input_query(uip: UIProps, queryfunc: str | None,
    ...
    TODO: Add support for asking multiple queries and/or having multiple fields.

        Parameters:
            uip (UIProps): The UIProps for the caller
            queryfunc (str): The query type; valid functions are:
                             int, str (will use an inputbox)
                             confirm (will use a boolean statement)
                             filechooser (will use a file chooser)
            queryval (str): The variable to query a value for
            query (str): The query to show
            selection_vars (dict[str, Any)): A dict to store the result in
            confirm (bool): Ask for confirmation before performing action?
            extravars (dict[str, Any]): Extra variables
            description (str): Description of action to use in confirmation dialog
        Returns:
            (bool): True if valid, False if aborted or invalid
    """
    ...
    if queryfunc is not None and queryval is not None and query is not None:
        if queryfunc in ("int", "string"):
            ...
            # querydefault = ""
            query_title: str = f"{query}: "
            if not (result := curses_helper.inputbox(uip.stdscr, title=query_title)):
                return False
            if queryfunc == "int":
                try:
                    result = int(result)
                except ValueError:
                    message = f"Provided value for {queryval} is not an integer"
                    curses_helper.alert(uip.stdscr, message=message,
                                        wait_for_keypress=True)
                    return False
            deep_set(selection_vars, DictPath(f"{queryval}"), result, create_path=True)
            ...
            listgetter = deep_get(extravars, DictPath("listgetter"))
            listgetter_args = deep_get(extravars, DictPath("listgetter_args"), {})
            basedir = deep_get(extravars, DictPath("basedir"))
            selected_file = None
            selected_ptype = None
            ...
            while basedir is not None:
                if not listgetter:
                    return False
                tmp_file_list = listgetter(basedir=basedir, **listgetter_args)
                file_list = []
...
def genericlistloop(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    global defaultlistaction  # pylint: disable=global-statement
    global defaultpath  # pylint: disable=global-statement
    ...
    helptext = generate_helptext(view, "listview", is_namespaced=is_namespaced)
    ...
    infogetter_extra_args: dict[str, Any] = {"_view": view.strip("*")}
    ...
    while True:
        if listgetter_async is not None:
            ...
            if listgetter_async in (listgetters_async.get_inventory_list,
                ...
                executor.trigger("pings", 5, run_ansible_play_async,
                                 playbook=facts_ping_playbook_path, selection=hosts,
                                 verbose=False, save_logs=False)
        ...
        if executor:
            if (execution_result_dict := executor.get("hosts")) != ([], []):
                ...
                # Update ansible groups, in case the groups have changed.
                for i, d in enumerate(vlist):
                    host = deep_get(d, DictPath("name"))
                    vlist[i]["ansible_groups"] = ansible_get_groups_by_host(inventory_dict, host)

                # Tag hosts in vlist that are no longer part of hosts as deleted.
                for host in vlist_hosts_set - hosts_set:
                    for i, d in enumerate(vlist):
                        if deep_get(d, DictPath("name"), "") == host:
                            vlist[i]["status"] = "DELETED"
                            vlist[i]["__deleted"] = True
                        vlist[i]["ansible_groups"] = \
                            ansible_get_groups_by_host(inventory_dict, host)
                infogetter_extra_args["_vlist"] = copy.deepcopy(vlist)
                if not uip.listlen:
                    uip.force_update()
                new_data = "pending"
        ...
        if not uip.is_list_regenerated():
            ...
            for y in range(uip.yoffset,
                ...
                is_tagged = in_tagged_items(item)
                is_deleted = deep_get(item, DictPath("__deleted"), False)
        ...
        if c == curses.KEY_F2:
            ...
            if (retval := selectwindow(uip, refresh_apis="all")) == Retval.RETURNFULL:
                return retval
            uip.refresh_all()
        elif c == ord(""):
            if (retval := selectwindow(uip, refresh_apis="selected")) == Retval.RETURNFULL:
                return retval
            uip.refresh_all()
        elif c == ord("B"):
            uip.toggle_borders()
            ...
            uip.force_update()
            uip.update_window(update="false")
        elif defaultlistaction or c == curses.KEY_F7:
            action_title: str = "Global and cluster-wide actions"
            ...
            # We've regenerated the field_dict and field_names,
            # so we need to update the infogetter_extra_args;
            # After that we need to empty uip.info and trigger a new update.
            infogetter_extra_args["_field_names"] = field_names
            infogetter_extra_args["_field_dict"] = field_dict
            uip.update_info([])
            update_field_widths(field_dict, field_names, uip.info)
            ...
            for listitem in uip.sorted_list:
                try:
            ...
            selected_name: str = deep_get(selected, DictPath("name"))
            ...
            for skip in ["all", "controlplane", "controlplanes", "master", "node", "nodes"]:
                try:
                    ...
                    pass
            host_groups = ansible_get_groups_by_host(inventory, selected_name)
            grouplist = []
            preselection = set()
            ...
            selection = \
                curses_helper.windowwidget(uip.stdscr, uip.maxy, uip.maxx,
                                           ...
                                           title=f"Toggle Ansible groups for {selected_name}:",
            ...
            if selection:
                ...
                for group in add_groups:
                    retval = ansible_add_hosts(inventory=ANSIBLE_INVENTORY,
                                               hosts=[selected_name], group=group, skip_all=False)
                for group in remove_groups:
                    retval = ansible_remove_hosts(inventory=ANSIBLE_INVENTORY,
                                                  hosts=[selected_name], group=group)

                if add_groups or remove_groups:
                    uip.force_update()
                    executor.retrigger("hosts")
            uip.refresh_all()
            curses.doupdate()
            ...
            for tagged_item in get_tagged_objects(uip.sorted_list):
                taggeditemlist.append({
                    "lineattrs": WidgetLineAttrs.NORMAL,
                    "columns": [[ThemeStr(f"{bullet}{deep_get(tagged_item, DictPath('name'))}",
                                          ThemeAttr("windowwidget", "default"))]],
                })
        ...
        # pylint: disable-next=too-many-nested-blocks
            ...
            for item in get_tagged_objects(uip.sorted_list):
                ...
                if not deep_get(item, DictPath("name")) \
                        or deep_get(item, DictPath("name")) == "<none>":
                    errmsg = [
                        [("Kind ", "default"),
                         (f"{kind}", "argument"),
                ...
                roles = deep_get(item, DictPath("roles"))
                if roles:
                ...
                if "namespace" in item:
                    namespace = deep_get(item, DictPath("namespace"), "")
                    items.append((namespace, name))
                else:
                    items.append(name)
        ...
        for key, value in shortcuts.items():
            ...
            if action is not None:
                ref = deep_get(selected, DictPath("ref"))
                action_args = deep_get(value, DictPath("action_args"), {})
                if "_pass_obj" in action_args and selected:
                    action_args["obj"] = ref
                ...
                if "_named_title" in action_args and selected is not None:
                    ...
                    if "_name_path" in action_args:
                        ...
                        title_name = deep_get_str_tuple_paths(ref, title_name_paths, "")
                    elif selected is not None:
                        title_name = deep_get(selected, DictPath("name"), "")
                    if title_name:
                ...
                if action == "cycle_var":
                    var = deep_get(action_args, DictPath("var"))
                    var_options = deep_get(action_args, DictPath("var_options"))
                    if var is not None:
                        tmp = deep_get(listview_args, DictPath(var))
                    ...
                    # Use this to show help entries for built-ins.
                    pass
    ...
    if keypress == curses.KEY_RIGHT:
        ...
        if oldval < maxval:
            for i in range(selected + 1, len(info)):
                if deep_get(info[i], DictPath("status_group")) < StatusGroup.OK:
                    selected = i
                    uip.force_update()
                    break
        ...
        if oldval > 0:
            for i in range(1, selected):
                if deep_get(info[selected - i], DictPath("status_group")) < StatusGroup.OK:
                    selected -= i
                    uip.force_update()
                    break
...
# pylint: disable-next=too-many-locals
def __list_control_planes(**kwargs: Any) -> tuple[Retval, dict]:
    uip = deep_get(kwargs, DictPath("uip"))
    nodeinfo = deep_get(kwargs, DictPath("nodeinfo"), [])

    control_planes = []

    for d in nodeinfo:
        node = deep_get(d, DictPath("ref"))

        is_control_plane: bool = False
        labels: dict[str, Any] = deep_get(node, DictPath("metadata#labels"), {})

        if deep_get(labels, DictPath("node-role.kubernetes.io/control-plane")) is not None \
                or deep_get(labels, DictPath("node-role.kubernetes.io/master")) is not None:
            is_control_plane = True
        if not is_control_plane:
            continue

        nodename = deep_get(node, DictPath("metadata#name"), "")
        addresses = deep_get(node, DictPath("status#addresses"), [])
        hostname, iips, eips = infogetters.get_node_addresses(addresses)

        iips_str = ".".join(iips)

        if iips_str == "<none>":
            iips_fmt = ThemeAttr("windowwidget", "dim")
        else:
            iips_fmt = ThemeAttr("windowwidget", "default")

        eips_str = ".".join(eips)

        if iips_str == "<none>":
            eips_fmt = ThemeAttr("windowwidget", "dim")
        else:
            eips_fmt = ThemeAttr("windowwidget", "default")

        control_planes.append({
            "lineattrs": WidgetLineAttrs.NORMAL,
            "columns": [[ThemeStr(f"{nodename}", ThemeAttr("windowwidget", "default"))],
                        [ThemeStr(f"{hostname}", ThemeAttr("windowwidget", "default"))],
                        [ThemeStr(iips_str, iips_fmt)],
                        [ThemeStr(eips_str, eips_fmt)]],
        })
    ...
    headers = ["Node:", "Hostname:", "Internal IPs:", "External IPs:"]
    curses_helper.windowwidget(uip.stdscr, uip.maxy, uip.maxx, uip.maxy // 2, uip.maxx // 2,
                               control_planes, title=title, headers=headers, cursor=False)
    return Retval.MATCH, {}
...
def __ssh_to_node(**kwargs: Any) -> tuple[Retval, dict]:
    uip = deep_get(kwargs, DictPath("uip"))
    node = deep_get(kwargs, DictPath("node"))
...
clusteroverview_shortcuts = {
    ...
    "List Control Planes": {
        "helptext": ("[Shift] + C", "Show list of Control Planes"),
        "shortcut": [ord("C")],
        "helpgroup": 2,
        "action": "key_callback",
        "action_call": __list_control_planes,
    },
...
# pylint: disable-next=unused-argument
def read_file_async(*args: Any, **kwargs: Any) \
        -> tuple[dict[str, Any] | list[str], StatusGroup | str]:
    """
    ...
    status: StatusGroup | str = StatusGroup.OK
...
# pylint: disable-next=unused-argument
def run_ansible_play_async(*args: Any, **kwargs: Any) -> tuple[dict[str, Any], int]:
    """
    ...
    """
    playbook: FilePath = deep_get(kwargs, DictPath("playbook"))
    selection: list[str] = deep_get(kwargs, DictPath("selection"), [])
    ...
    status, result = ansible_run_playbook_on_selection(playbook=playbook,
                                                       selection=selection,
                                                       verbose=verbose, save_logs=save_logs)
    return result, status
...
def clusteroverviewloop(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    # For generic information + events
    uip.init_infopad(height=infopadheight + eventpadheight + 1, width=-1, ypos=infopadypos, xpos=1)
    ...
    control_planes: list[tuple[dict[str, Any], str]] = get_control_planes()
    control_plane_names: list[str] = [control_plane[1] for control_plane in control_planes]
    control_plane_name: str = control_plane_names[0]
    control_plane_objs: list[dict[str, Any]] = \
        [control_plane[0] for control_plane in control_planes]

    if not control_plane_name:
        control_plane_names = ["<virtual>"]
        control_plane_name = "<virtual>"
        control_plane_objs = []
    ...
    for obj in control_plane_objs:
        addresses = deep_get(obj, DictPath("status#addresses"), [])
        name_, iips_, eips_ = infogetters.get_node_addresses(addresses)
        if not eips or eips_ != ["<none>"]:
            eips += eips_
        if not iips or iips_ != ["<none>"]:
            iips += iips_
    ...
    node_heatmap: list[list[ThemeRef | ThemeStr]] = []
    pod_statuses: list[StatusGroup] = []
    pod_heatmap: list[list[ThemeRef | ThemeStr]] = []
    ...
    while True:
        ...
        if (result := executor.get("node")) != ([], []):
            nodeinfo, node_statuses = cast(tuple[list[Any], list[Any]], result)
        if (result := executor.get("pod")) != ([], []):
            podinfo, pod_statuses = cast(tuple[list[Any], list[Any]], result)
        if (result := executor.get("event")) != ([], []):
            events, _event_status = cast(tuple[list[dict[str, Any]], list[Any]], result)
        ...
        if uip.is_update_triggered():
            ...
            if stat_raw:
                match_tmp = stat_regex.match(stat_raw[0])
                if match_tmp is not None:
                    cputimeuser = int(match_tmp[1])
                    cputimeusernice = int(match_tmp[2])
                    cputimesystem = int(match_tmp[3])
                    cputimeidle = int(match_tmp[4])
                    cputimeiowait = int(match_tmp[5])
                    cputimeirq = int(match_tmp[6])
                    cputimesoftirq = int(match_tmp[7])
                    cputimesteal = int(match_tmp[8])
                    cputimeguest = int(match_tmp[9])
                    cputimeguestnice = int(match_tmp[10])
            ...
            pod_heatmap_right_margin = 2 if CursesConfiguration.borders else 0
            node_heatmap_width = ((uip.maxx - uip.minx - 1) // 5) - 2
            pod_heatmap_width = ((uip.maxx - uip.minx - 1) * 4 // 5) - pod_heatmap_right_margin

        k8s_distro, _status = kh.identify_k8s_distro()
        _1, _2, server_git_version = kh.get_api_server_version()

        if k8s_distro is not None:
            k8s_distro_string: ThemeStr | ThemeRef = ThemeStr(f"{k8s_distro}",
                                                              ThemeAttr("types", "generic"))
        else:
        ...
        if iips == ["<none>"]:
            ...
            internal_ips_array = \
                generators.format_address(iips, selected=False, fieldlen=0, pad=False)
        if eips == ["<none>"]:
            external_ips_array: list[ThemeRef | ThemeStr] = \
                [ThemeStr("<none>", ThemeAttr("types", "none"))]
        else:
            external_ips_array = \
                generators.format_address(eips, selected=False, fieldlen=0, pad=False)

        if control_plane_names == ["<virtual>"]:
            control_plane_list: list[ThemeRef | ThemeStr] = \
                [ThemeStr("<virtual>", ThemeAttr("types", "unset"))]
        else:
            control_plane_list = \
                generators.format_list(control_plane_names, fieldlen=0, pad=False)

        infoarrays: list[list[ThemeStr] | list[ThemeRef | ThemeStr]] = [
            [
                ThemeStr("C", ThemeAttr("main", "infoheader_shortcut")),
                ThemeStr("ontrol Planes: ", ThemeAttr("main", "infoheader")),
            ] + control_plane_list, [
        ...
        for i, row in enumerate(infoarrays):
            uip.addthemearray(uip.infopad, cast(list[ThemeRef | ThemeStr], row), y=y + i, x=0)
        y += i

        percentagebar_margin: int = 9 if CursesConfiguration.borders else 7
        ...
        percentagebar_cpu: list[ThemeRef | ThemeStr] = \
            curses_helper.percentagebar(7, uip.infopadwidth - percentagebar_margin, cputimetotal, [
        ...
        percentagebar_mem: list[ThemeRef | ThemeStr] = \
            curses_helper.percentagebar(7, uip.infopadwidth - percentagebar_margin, memtotal, [
        ...
        if swaptotal:
            percentagebar_or_string_swap: list[ThemeRef | ThemeStr] = [
                ThemeStr("[", ThemeAttr("types", "generic"))
            ] + curses_helper.percentagebar(7, uip.infopadwidth - percentagebar_margin, swaptotal,
                                            [(swapused, ThemeRef("strings", "swap_used"))]) + [
                ThemeStr("]", ThemeAttr("types", "generic")),
                ThemeStr(f"{str(100 * swapused // swaptotal).rjust(4)}",
                         ThemeAttr("types", "numerical")),
                ThemeRef("separators", "percentage"),
            ]
        else:
            percentagebar_or_string_swap = [
                ThemeStr("Disabled", ThemeAttr("types", "none")),
            ]
        ...
        tasksarrays: list[list[ThemeRef | ThemeStr]] = [
            [
                ...
                ThemeStr("]", ThemeAttr("types", "generic")),
                ThemeStr(f"{str(100 * cputimetotalused // cputimetotal).rjust(4)}",
                         ThemeAttr("types", "numerical")),
                ThemeRef("separators", "percentage"),
            ], [
                ...
                ThemeStr("]", ThemeAttr("types", "generic")),
                ThemeStr(str(100 * memused // memtotal).rjust(4),
                         ThemeAttr("types", "numerical")),
                ThemeRef("separators", "percentage"),
            ], [
        ...
        # Draw the events over the bottom of the visible area of the infopad
        eventarrays: list[list[ThemeRef | ThemeStr]] = []

        tmp2: list[ThemeRef | ThemeStr] = []
        ...
        for evindex in range(0, 5):
            ...
            if evindex < len(events):
                ...
                for i, evfield in enumerate(evfields):
                    ...
                    elif ptype == "event_reason":
                        reason_format = deep_get(event_reasons, DictPath(val),
                                                 ThemeAttr("main", "default"))
                        tmp2.append(ThemeStr(val.ljust(evlens[i]), reason_format))
                    else:
        ...
        curses_helper.window_tee_hline(uip.stdscr, y=y, start=0, end=uip.maxx,
                                       tee=CursesConfiguration.borders)
        if uip.infopad:
            curses_helper.window_tee_hline(uip.infopad, y=y - 1, start=0, end=uip.maxx, tee=False)

        # Draw the events over the hline
        eventarray_header: list[ThemeRef | ThemeStr] = [
            ThemeStr("E", ThemeAttr("main", "listheader_compact_shortcut")),
            ThemeStr("vents:", ThemeAttr("main", "listheader_compact")),
        ]
        uip.addthemearray(uip.infopad, eventarray_header, y=y - 1, x=0)
        ...
        if nodeinfo or podinfo:
            node_heatmap_xpos = 0
            pod_heatmap_xpos = uip.maxx - \
                pod_heatmap_width - 2 + (2 - pod_heatmap_right_margin)

            taints: list[str] = []
            if nodeinfo:
                taints = deep_get(nodeinfo[selected_node], DictPath("taints"), ["<none>"])
            if not taints:
                taints = ["<none>"]
            selectednodetaints = \
            ...
            if nodeinfo:
                node_name = deep_get(nodeinfo[selected_node], DictPath("name"))
                node_status = deep_get(nodeinfo[selected_node], DictPath("status"))
                node_status_group = deep_get(nodeinfo[selected_node], DictPath("status_group"))
            ...
            if podinfo:
                pod_name = deep_get(podinfo[selected_pod], DictPath("name"))
                pod_namespace = deep_get(podinfo[selected_pod], DictPath("namespace"))
                pod_node = deep_get(podinfo[selected_pod], DictPath("node"))
                pod_status = deep_get(podinfo[selected_pod], DictPath("status"))
                pod_status_group = deep_get(podinfo[selected_pod], DictPath("status_group"))

            if pod_node == "<none>":
                pod_node_array: list[ThemeRef | ThemeStr] = \
                    [ThemeStr(f"{pod_node}".ljust(pod_heatmap_width), ThemeAttr("types", "none"))]
            else:
                pod_node_array = \
            ...
            podarrays: list[list[ThemeRef | ThemeStr]] = [
                [
                    ...
                    ThemeStr("d", ThemeAttr("main", "listheader"),
                             selected=selected_heatmap == "Pod"),
                    ThemeStr("e: ", ThemeAttr("main", "infoheader"))] + pod_node_array + [
                ], [
                    ThemeStr(" ", ThemeAttr("main", "listheader")),
                    ThemeStr("Status: ", ThemeAttr("main", "infoheader")),
        ...
        if podinfo and selected_heatmap == "Pod":
            __node_name = deep_get(podinfo[selected_pod], DictPath("node"))
            __pod_name = deep_get(podinfo[selected_pod], DictPath("name"))
            __namespace = deep_get(podinfo[selected_pod], DictPath("namespace"))
            __info = podinfo
            __selected = selected_pod
            heatmap_width = pod_heatmap_width
        elif nodeinfo and selected_heatmap == "Node":
            __node_name = deep_get(nodeinfo[selected_node], DictPath("name"))
            __namespace = None
            __pod_name = None
...
def decode_and_view_data(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    if "selected" in kwargs:
        ...
        bvtype = deep_get(kwargs, DictPath("selected#vtype"))
        vtype, value = decode_value(bvalue)
        if bvalue == value:
            vtype = bvtype
    elif "obj" in kwargs and "value" in kwargs["obj"]:
        bvalue = deep_get(kwargs, DictPath("obj#value"))
        title = deep_get(kwargs, DictPath("obj#key"))
        bvtype = deep_get(kwargs, DictPath("obj#vtype"))
        vtype, value = decode_value(bvalue)
        if bvalue == value:
            vtype = bvtype
    elif "path" in kwargs:
    ...
    if vtype.startswith("string"):
        ...
        obj = base64.b64decode(value).decode("utf-8", errors="replace")
    else:
        errmsg = [
    ...
    elif title in (".dockercfg", ".dockerconfigjson", "v4-0-config-system-session", "userData") \
            or title.endswith((".kubeconfig", ".json")):
        formatter = formatters.reformat_json
    elif title in ("kubeconfig", "kubeConfig", "HelmChartValuesContent") or title.endswith(".yaml"):
        formatter = formatters.format_yaml
    elif title == "fluent-bit.conf":
...
def export_data(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    if selected is not None:
        value = deep_get(selected, DictPath(name_attr))
    if obj is not None:
        if value is not None:
            value = deep_get(obj, DictPath(f"{base_path}#{value}"))
        else:
            if base_path is not None and not base_path:
                value = obj
            else:
    ...
    if not isinstance(value, (str, bytes)):
        errmsg = [
            [("export_data()", "emphasis"),
             (" received incompatible value type:", "error")],
            [("type: ", "default"),
             (f"{type(value)}", "argument"),
             (" (expected one of ", "default"),
             ("str", "argument"),
             (", ", "default"),
             ("bytes", "argument"),
        ...
        curses_helper.alert(stdscr,
                            message=f"Failed to export data; unsupported value type {type(value)}",
                            wait_for_keypress=True)
        return Retval.RETURNDONE

    if not (raw_filename := deep_get(kwargs, DictPath("result"))):
        return Retval.RETURNDONE
    ...
    if filename.startswith(f"~{os.path.sep}"):
        filename = FilePath(f"{HOMEDIR}{os.path.sep}{filename[2:]}")
    ...
    if vtype.startswith("string") or raw_export:
        ...
        secure_write_string(filename,
                            base64.b64decode(value).decode("utf-8", errors="replace"))
    elif vtype.startswith(("base64-binary", "gzip")):
        secure_write_string(filename, base64.b64decode(value),
                            write_mode="wb")
    else:
        errmsg = [
            [("export_data()", "emphasis"),
...
# pylint: disable-next=too-many-branches,too-many-statements
def generate_helptext(view: str | tuple[str, str], viewtype: str, **kwargs: Any) -> list[dict]:
    """
    Generate helptext for a view.
        ...
        Parameters:
            ...
            **kwargs (dict[str, Any]): Keyword arguments
                ...
                is_namespaced (bool): Is this resource namespaced?
        Returns:
    ...
    additional_helptexts: list[tuple[str, str]] = \
        deep_get(kwargs, DictPath("additional_helptexts"), [])
    shortcuts: dict[str, dict[str, Any]] = deep_get(kwargs, DictPath("shortcuts"), {})
    is_namespaced: bool = deep_get(kwargs, DictPath("is_namespaced"), False)
    ...
    if viewtype == "infoview":
        ...
        if is_namespaced:
            helptext += helptexts.selectnamespace
...
def process_selection(**kwargs: Any) -> dict[str, Any]:
    ...
    Process selected values.
        ...
        Parameters:
            **kwargs (dict[str, Any]): Keyword arguments
                process_selection (dict[int, Any]): A lookup table for substitution
                                                    the output from input selections.
    """
    process_selection_dict: dict[int, dict[str, dict[str, Any]]] = \
...
def genericinfoloop(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    title_path: str = deep_get(kwargs, DictPath("title_path"), "")
    title = f"{title}{deep_get(obj, DictPath(title_path), '')}"

    if (obj_path := deep_get(kwargs, DictPath("obj_path"))):
        obj = deep_get(obj, DictPath(obj_path))

    if isinstance(formatter, str):
        if formatter == "identify":
            _dataformat, formatter = formatters.identify_cmdata(cmdata_name=title, cm_name="",
    ...
    field_indexes = deep_get(viewref, DictPath("field_indexes"), {})
    if "Wide" in field_indexes:
        field_index: str = "Wide"
    else:
        field_index = "Normal"
    ...
    # On the one hand we want to be able to override defaults,
    # but on the other hand we also want to be able to toggle values.
    ...
    if formatter:
        ...
        if not view[1]:
            ...
            kind_joined = ".".join(view)
        # Apply user configuration
        for key, value in formatter_args.items():
            if key in deep_get(cmtlib.cmtconfig, DictPath(f"{kind_joined}#formatter_args"), {}):
                formatter_args[key] = \
    ...
    infopadheight = 0
    conditions_path: str = "status#conditions"
    ...
    if "infopad" in viewref:
        ...
        conditions_path = deep_get(viewref, DictPath("conditions_path"), "status#conditions")
    ...
    # unless the path returns None
    if deep_get(obj, DictPath(conditions_path)) is not None:
    ...
    helptext = generate_helptext(view, "infoview",
                                 additional_helptexts=additional_helptexts, shortcuts=shortcuts)
    ...
    while True:
        ...
        if uip.is_update_triggered():
            ...
            if not view[0].startswith("__") \
                ...
                obj_ = kh.get_ref_by_kind_name_namespace(view, obj_name, obj_namespace,
            ...
            if uip.infopad is not None:
                ...
                if creation_timestamp_path is not None \
                        ...
                        and deep_get(obj, DictPath(creation_timestamp_path)) is not None:
                    timestamp: str | datetime = deep_get(obj, DictPath(creation_timestamp_path))
            ...
            if uip.listpad is not None:
                ...
                _set_based_selector_path = \
                    deep_get(listgetter_args, DictPath("_set_based_selector_path"))
                if _set_based_selector_path:
                    set_expression = \
                        make_label_selector_set_expression(deep_get(obj, _set_based_selector_path))
                    if deep_get(listgetter_args, DictPath("label_selector")):
                        listgetter_args["label_selector"] += "," + set_expression
                    else:
                        listgetter_args["label_selector"] = set_expression
                ...
                if uip.yoffset + y < len(facilities):
                    ...
                    if fac:
                        if isinstance(fac, tuple):
                            ...
                            if isinstance(fac[0], list):
                                fac_extended: list[ThemeRef | ThemeStr] = fac[0]
                            else:
                                fac_extended = [ThemeStr(fac[1],
                                                ThemeAttr("types", "facility_extended"))]
                            fac_regular: str = fac[1]
                        else:
                            fac_extended = []
                            fac_regular = fac
                        ...
                        if not fac_regular.strip():
                            facility += [ThemeStr("".ljust(facility_prefix_len),
                                                  ThemeAttr("logview", "facility")),
                                         ThemeStr(fac_regular, ThemeAttr("logview", "facility")),
        ...
        if initial_container is not None:
            ...
            if uip.info:
                ...
                for item in uip.info:
                    ...
                    ikind = deep_get(item, DictPath("type"))
                    if ikind in ("[container]", "[init_container]", "[configmap]"):
                        ...
                        if isinstance(ikind, str):
                            ...
                            and deep_get(item, DictPath("vtype")) in ("base64-utf-8",
                                                                      "base64-utf-8 [truncated]",
                                                                      "string"):
                        name = deep_get(item, DictPath("key"))
                        ikind = ("Secret", "")
                    else:
                        continue
                    ...
                    if name == initial_container:
                        if view == ("ConfigMap", ""):
                            match = deep_get(item, DictPath("data"))
                        elif view == ("Secret", ""):
                            match = item
                        else:
                            match = deep_get(item, DictPath("ref"))
                        match_count = 1
                        break
        ...
        if c == curses.KEY_F3:
            ...
            continue
        if c == ord(""):
            # Reload the selected view-file, and if we're in the pod or podlog view,
            # also the parser-files.
            if view in (("Pod", ""), ("__PodLogView", "")):
                init_parser_list(force_reinit=True)
            if (retval := selectwindow(uip, refresh_apis="selected")) == Retval.RETURNFULL:
                return retval
            uip.refresh_all()
            continue
        ...
        for _key, sc_value in shortcuts.items():
            ...
            if widget == "windowwidget":
                ...
                if w_itemgetter_src == "selected":
                    selected = uip.get_selected()
                    if selected is not None:
                        w_ref = deep_get(selected, DictPath("ref"), None)
            ...
            if "action" in sc_value:
                ...
                if "_pass_selected_obj" in action_args and selected is not None:
                    action_args["selected_obj"] = deep_get(selected, DictPath("ref"), None)
                if "_parent_kind_tuple_paths" in action_args and selected is not None:
                    parent_kind_path, parent_api_version_path = \
                        deep_get(action_args, DictPath("_parent_kind_tuple_paths"))
                    parent_kind = deep_get(obj, parent_kind_path, "")
                    parent_api_version = deep_get(obj, parent_api_version_path, "")
                    action_args["_parent_kind_tuple"] = (parent_kind, parent_api_version)
                if "_parent_name_path" in action_args:
                    parent_name_path = \
                        deep_get(action_args, DictPath("_parent_name_path"))
                    action_args["_parent_name"] = \
                        deep_get(obj, DictPath(parent_name_path), "")
                ...
                if "_named_title" in action_args:
                    ...
                    title_name: str = ""
                    if selected is not None:
                        ref = deep_get(selected, DictPath("ref"), None)
                ...
                elif action == "cycle_var":
                    var = deep_get(action_args, DictPath("var"))
                    var_options = deep_get(action_args, DictPath("var_options"))
                    if var is not None:
                        tmp = deep_get(infoview_args, DictPath(var))
                        deep_set(infoview_args, DictPath(var), next_option(tmp, var_options))
                elif action == "execute":
                    _command = deep_get(action_args, DictPath("command"), [])
                    newlines = deep_get(action_args, DictPath("newlines"), "keep")
                    replace_quotes = deep_get(action_args, DictPath("replace_quotes"), "keep")
                    # replace paths with data
                    if _command:
                        command = []
                        for _cmd in _command:
                            if isinstance(_cmd, list):
                                value = deep_get_with_fallback(obj, _cmd)
                                if isinstance(value, str):
                                    value = cmtlib.replace_newlines(value, policy=newlines)
                                    value = cmtlib.replace_quotes(value, policy=replace_quotes)
...
def eventdispatch(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    if "/" not in api_version:
        ...
        api_family = api_version.split("/", maxsplit=1)[0]
...
def log_add_line(timestamps: list[datetime],
    ...
    Add a new line to the log.
        ...
        Parameters:
            ...
            facilities ([([ThemeRef | ThemeStr], str) | str]): The list of severities
            severities ([LogLevel]): The list of severities
            messages ([[ThemeRef | ThemeStr] | str]): The list of log messages
            timestamp (datetime | None): The timestamp of the message to add
    ...
    if isinstance(message, str):
        ...
        tabsplit = message.split("\t")
        if len(tabsplit) > 1:
            new_message = ""
            tab = ""
            for segment in tabsplit:
                new_message += tab
                new_message += segment
                tabsize = len(new_message + segment) % 8
                if not tabsize:
                    tabsize = 8
                tab = "".ljust(tabsize)
...
def containerinfoloop(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    show_borders = deep_get(cmtlib.cmtconfig, DictPath("Global#borders"), True)
    ...
    while True:
        if uip.is_update_triggered():
            ...
            if not multilog_containers:
                ...
                if kind == ("InitContainer", ""):
                    src_statuses = \
                        deep_get(pod_info_ref, DictPath("status#initContainerStatuses"), [])
            ...
            if not multilog_containers:
                ...
                for namespace, podname, container_type, \
                    ...
                    containerinfolist.append({
                        "lineattrs": WidgetLineAttrs.NORMAL,
                        "columns": [[ThemeStr(namespace, ThemeAttr("windowwidget", "default"))],
                                    [ThemeStr(podname, ThemeAttr("windowwidget", "default"))],
                                    [ThemeStr(container_type,
                                              ThemeAttr("windowwidget", "default"))],
                                    [ThemeStr(containername, ThemeAttr("windowwidget", "default"))],
                                    [ThemeStr(image, ThemeAttr("windowwidget", "default"))],
                                    [ThemeStr(image_id, ThemeAttr("windowwidget", "default"))]],
                        "retval": (containername, image_id),
                    })
                    ...
                    for line in split_msg(rawmsg):
                        splitmsg.append((line, podname, containername, image, facility_extended))
                splitmsg = cast(list[tuple[str, str, str, str, list[ThemeRef | ThemeStr]]],
            ...
            prev_message: list[ThemeRef | ThemeStr] = \
                cast(list[ThemeRef | ThemeStr], [ThemeStr("", ThemeAttr("types", "generic"))])
            prev_remnants: list = []
            repeat_count: int = 0
            message: list[ThemeRef | ThemeStr] \
                     | tuple[str, Callable[..., Any] | None, dict[Any, Any]] \
                     | list[tuple[list[ThemeRef | ThemeStr], LogLevel]] = []
            facility: tuple[list[ThemeRef | ThemeStr], str] | str = ""
            _facility: tuple[list[ThemeRef | ThemeStr], str] | str = ""
            ...
            if not multilog_containers:
                ...
                if kind == ("InitContainer", ""):
                    ...
                    src_statuses = deep_get(pod_info_ref, DictPath("status#containerStatuses"))
            ...
            for y in range(0, min(uip.logpadheight, uip.loglen)):
                ...
                if show_facility == "None" or uip.yoffset + y >= len(facilities):
                    ...
                    if _facility is None:
                        _facility = ""
                    match_tmp = facility_regex.match(_facility)
                    if match_tmp is not None:
                        facility = match_tmp[1]
        ...
        uip.stdscr.timeout(100)
        c = uip.stdscr.getch()
        retval: Retval | None = uip.generic_keycheck(c)
        ...
        if c == curses.KEY_F2:
            ...
            retval = None
            if multilog_containers and len(multilog_containers) > 1:
                containerheaders: list[str] = ["Namespace:", "Pod:", "Type:", "Container:",
                                               "Image:", "Image ID:"]
                # List containers (and upon selection display
                # information about the selected container)
                tmpselection = curses_helper.windowwidget(uip.stdscr, uip.maxy, uip.maxx,
                                                          uip.maxy // 2, uip.maxx // 2,
                                                          containerinfolist,
                                                          headers=containerheaders,
                                                          title="Containers", cursor=True)
                uip.refresh_all()
                if not tmpselection:
                    continue
                containername, image_id = tmpselection

            # XXX: This is very inefficient
            _vlist = infogetters.get_container_info(kubernetes_helper=kh, kh_cache_=kh_cache)
            for _obj in _vlist:
                _obj_name = deep_get(_obj, DictPath("name"))
            ...
            for i, option in enumerate(option_list):
                ...
                if option_path is not None:
                    option_retval = getattr(LogparserConfiguration, option_path, False)
                if option_retval and isinstance(preselection, set):
                    preselection.add(i)
            ...
            if selection is not None \
                    and preselection != selection and isinstance(preselection, set):
                # Perform the changes
            ...
            for parser_name in natsorted(get_parser_list()):
                parserlist.append({
                    "lineattrs": WidgetLineAttrs.NORMAL,
                    "columns": [[ThemeStr(f"{parser_name}", ThemeAttr("windowwidget", "default"))]],
                    "retval": parser_name,
                })
            ...
            if filename.startswith(f"~{os.path.sep}"):
                filename = f"{HOMEDIR}{os.path.sep}{filename[2:]}"
            ...
            for y in range(0, uip.loglen):
                ...
                if y >= len(facilities) or not facilities[y]:
                    facility = ""
                else:
                    tmp_facility_raw = facilities[y]

                    # This is facility_extended + facility
                    if isinstance(tmp_facility_raw, tuple):
                        # It seems that mypy doesn't understand that facilities[y] can
                    ...
                    elif not tmp_facility_raw.strip():
                ...
                if raw_logs:
                    ...
                    data += f"{timestamp}  {severity_str}{facility}{tmp_message}\n"
...
def do_command(stdscr: curses.window, **kwargs: Any) -> None:
    ...
    _args: list[str | list[str]] = deep_get(kwargs, DictPath("command"), [])
    ...
    for arg in _args:
        if isinstance(arg, str):
            ...
            # For now we only support joining
            tmp = ""
            for arg2 in deep_get(arg, DictPath("join"), []):
                if isinstance(arg2, str):
                    tmp += arg2
                if isinstance(arg2, list):
                    tmp += deep_get_with_fallback(obj, DictPath(arg2), "")
            args.append(tmp)
...
def patch_object(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    if not values and "selected" in kwargs:
        values = deep_get(kwargs, DictPath("selected"))
...
def resourceinfodispatch_with_lookup(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    the source object.
        ...
        Parameters:
            ...
            **kwargs (dict[str, Any]): Keyword arguments
                ...
                name_path (DictPath): The path to get name from
                name_regex (str): Regex to use to extract the name
                ...
                _parent_kind_tuple: Kind tuple from the parent object
                _parent_namespace: Namespace from the parent object
            ...
            if "_selected_objs" in kwargs:
                ...
                for key, attribute in deep_get(kwargs, DictPath("_selected_objs"), []).items():
                    if isinstance(attribute, list):
                        tmp: Any = deep_get(selected, DictPath(attribute[0]))
                        if tmp is not None:
                            obj[key] = tmp[attribute[1]]
                        else:
                            obj[key] = None
                    else:
                        obj[key] = deep_get(selected, DictPath(attribute))
            else:
                obj = selected
    ...
    if isinstance(selection, dict):
        if obj is None:
            obj = selection
        tmp_fields: list[Any] | tuple[Any, ...] = deep_get(selection, DictPath("fields"), [])
    ...
    name_regex = deep_get(kwargs, DictPath("name_regex"))
    namespace = deep_get(kwargs, DictPath("namespace"))
    ...
    if owner_reference_path is not None and obj is not None:
        ...
        must_be_controller: bool | None = deep_get(kwargs, DictPath("controller"))
        for ref in owner_references:
            ...
            if "/" in ref_api_version:
                ref_api_family = ref_api_version.split("/", maxsplit=1)[0]
            else:
                ref_api_family = ""
            ref_kind: tuple[str, str] = (deep_get(ref, DictPath("kind")), ref_api_family)
    ...
    parent_kind_tuple = deep_get(kwargs, DictPath("_parent_kind_tuple"), ("", ""))
    if parent_kind_tuple != ("", ""):
        parent_kind, parent_api_version = parent_kind_tuple
        if "/" in parent_api_version:
            parent_api_family = parent_api_version.split("/", maxsplit=1)[0]
        else:
            parent_api_family = ""
        kind = (parent_kind, parent_api_family)

    if kind is None or isinstance(kind, tuple) and kind[0] is None:
        return Retval.NOMATCH

    if namespaced_name_path is not None:
        namespaced_name = deep_get_with_fallback(obj, namespaced_name_path)
...
def run_playbook(playbook: dict, hosts: list[str], values: dict | None = None) -> int:
    """
    Run a playbook.
    ...
    if paths:
        ...
        if deep_get(kwargs, DictPath("include_root"), False):
            for tmppath in paths:
                _obj[tmppath.rsplit('#')[-1]] = deep_get(obj, DictPath(tmppath))
        else:
            _obj = deep_get_with_fallback(obj, paths)
        if not _obj:
            return Retval.RETURNDONE
        obj = _obj
    title = deep_get_with_fallback(kwargs, [DictPath("named_title"), DictPath("title")])
...
def view_pod_logs(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    namespaces = set()
    for obj in items:
        namespace = deep_get(obj, DictPath("namespace"))
        namespaces.add(namespace)
        name = deep_get(obj, DictPath("name"))
        container_name = deep_get(obj, DictPath("container"))
        ...
        if image.startswith("sha256:"):
            image = image_id
        container_type = deep_get(obj, DictPath("type"))
        containers.append((namespace, name, container_type, container_name))
        containers_full.append((namespace, name, container_type, container_name, image, image_id))
...
def action_view_file(**kwargs: Any) -> Retval:
    ...
    path, filetype = deep_get(values, DictPath("resource_path"), ("", ""))
    tmp = secure_read_string(path)
    data = {"data": tmp}
...
def update_version_cache(stdscr: curses.window, **kwargs: Any) -> Retval:
    ...
    networkio.update_version_cache(verbose=True, force=True)
...
def perform_action_on_configuration(uip: UIProps, **kwargs: Any) -> Retval:
    ...
    kubectl_major_version, kubectl_minor_version, *_ignore = kubectl_get_version(client_only=True)
...
def diff_resource_configuration(uip: UIProps, **kwargs: Any) -> Retval:
    ...
    for line in result.stdout.splitlines():
        ...
        if indent != 2:
            prefix = line[0]
            match_tmp = diff_regex.match(line[1:])
            if match_tmp is not None and len(match_tmp[1]) % 2 == 0:
                line = prefix + "".ljust(int(len(match_tmp[1]) / 2) * indent) + match_tmp[2]
...
def delete_logs(**kwargs: Any) -> Retval:
    ...
    if dirpath is not None:
        if dirpath.startswith("{HOME}"):
            dirpath = f"{HOMEDIR}{dirpath[len('.HOME.'):]}"
        if dirpath.startswith(f"~{os.path.sep}"):
            dirpath = f"{HOMEDIR}{os.path.sep}{dirpath[2:]}"
    for item in items:
...
def action_execute_command(uip: UIProps, **kwargs: Any) -> None:
    ...
    if not tmpargs or not tmpargs[0]:
        ansithemeprint([ANSIThemeStr("Critical", "critical"),
                        ANSIThemeStr(": Request to execute empty or None command; ", "default"),
                        ANSIThemeStr("aborting.", "default")],
                        stderr=True)
        tmpargs = []  # To skip the loop without special cases.
        retval = 2
    ...
    for arg in tmpargs:
        if arg is None:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ...
                            ANSIThemeStr(f"{tmpargs[0]}", "programname"),
                            ANSIThemeStr("“ Received a None argument; aborting.", "default")],
                            stderr=True)
            retval = 2
            break
        if arg == "<<<items>>>":
            args += cast(list[str], items)
        elif arg.startswith("<<<items:"):
            separator = arg.removeprefix("<<<items:").removesuffix(">>>")
            args.append(separator.join(cast(list[str], items)))
        else:
            args.append(arg)
    ...
    if not retval:
        ...
        if iterate:
            iterations: list[tuple[str, str] | str] = cast(list[tuple[str, str] | str], items)
        else:
            iterations = [cast(tuple[str, str] | str, items)]
        ...
        for item in iterations:
            ...
            try:
                ...
                ansithemeprint([ANSIThemeStr("Error", "error"),
                                ANSIThemeStr(": Command “", "default"),
                                ANSIThemeStr(f"{args[0]}", "programname"),
                                ANSIThemeStr("“ not found; aborting.", "default")],
                                stderr=True)
                retval = 2
...
def list_configuration_files(**kwargs: Any) -> list[tuple[str, str, str]]:
    """
    ...
    and directories with a kustomization.yaml file in it.
        ...
        Parameters:
            **kwargs (dict[str, Any]): Keyword arguments
                ...
                kubernetes_configuration (bool): Are we looking for a Kubernetes configuration?
                filetypes (dict[str, str]): Permitted suffixes; if empty all types are permitted,
                                            but none of them identified.
        Returns:
    ...
    basedir: FilePath = deep_get(kwargs, DictPath("basedir"), HOMEDIR)
    filetypes: dict[str, str] = deep_get(kwargs, DictPath("filetypes"), {})
    kubernetes_configuration: bool = deep_get(kwargs, DictPath("kubernetes_configuration"), False)

    try:
        for filename in Path(basedir).iterdir():
            if filename.name.startswith(("~", ".")):
                continue

            if kubernetes_configuration:
                # real path, display name, type
                if filename.suffix in (".yaml", ".yml"):
                    files.append((str(filename), filename.name, "Configuration File"))
                elif filename.is_dir():
                    if Path(filename / "kustomization.yaml").is_file():
                        files.append((str(filename), filename.name, "Kustomization"))
                    else:
    ...
    plist = [(str(Path(basedir).parent), "..", "<dir>")]
    plist += cast(list[tuple[str, str, str]], natsorted(files, key=itemgetter(2, 0)))
...
def populate_actionlist(**kwargs: Any) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    currentcategory = ""
    for action_item in natsorted(order, key=itemgetter(1, 0)):
        action = action_item[0]
        category = action_item[1]
        allowoncontrolplane = \
            deep_get(action_list, DictPath(f"{action_item[2]}#allow_on_control_plane"), True)
        singleoncontrolplane = \
            deep_get(action_list, DictPath(f"{action_item[2]}#single_on_control_plane"), False)
        singleonly = deep_get(action_list, DictPath(f"{action_item[2]}#single_only"), False)
        readonly = deep_get(action_list, DictPath(f"{action_item[2]}#read_only"), False)
        metadata = deep_get(action_list, DictPath(f"{action_item[2]}#metadata"), [])
        tmp = []
        for md in metadata:
...
def populate_playbooklist(**kwargs: Any) -> tuple[list[dict], dict]:
    ...
    """
    context: dict | None = deep_get(kwargs, DictPath("context"))
...
# pylint: disable-next=too-many-locals,too-many-branches,too-many-statements
def format_selection_list(uip: UIProps, refresh_apis: str = "none") -> list[dict[str, Any]]:
    """
    Format the selection list.
        ...
        Parameters:
            uip (UIProps): A reference to the UI Properties object
            **kwargs (dict[str, Any]): Keyword arguments
                refresh_apis (str): Should the list of available APIs be refreshed:
                                    all: Refresh all APIs
                                    selected: Refresh the selected view
                                    none: Do not refresh APIs
    """
    global force_refresh_apis  # pylint: disable=global-statement
    ...
    if refresh_apis != "none":
        ...
        if refresh_apis == "selected":
            # Reload the view-file; typically used during development
            _notice = curses_helper.notice(uip.stdscr, message="Reloading view-file")
        populate_views(refresh_apis=refresh_apis)
        curses.doupdate()
...
def selectwindow(uip: UIProps, **kwargs: Any) -> Retval:
    ...
    refresh_apis: str = deep_get(kwargs, DictPath("refresh_apis"), "none")
...
listviewactions: dict[str, dict[str, Any]] = {
    "View File": {
        ...
        "actionfunc": action_view_file,
        "actionfunc_args": {
            "formatter": "format_autodetect",
        },
        "query": "Choose file to view:",
...
action_call_allowlist: dict[str, Callable] = {
    ...
    "update_version_cache": update_version_cache,
...
# Actionfuncs acceptable for direct use in view files from actionlists
...
# pylint: disable-next=too-many-locals,too-many-branches,too-many-statements
def populate_views(refresh_apis: str = "none") -> None:
    """
    Populate the list- and info-view data from view-files.
        ...
        Parameters:
            refresh_apis (str): Should the list of available APIs be refreshed:
                                all: Refresh all APIs
                                selected: Refresh the selected view
                                none: Do not refresh APIs (does nothing if the views are populated)
    """
    global views  # pylint: disable=global-statement

    # We do not need to reload the list of available kinds if we're only
    # refreshing the selected API, since it's already available
    force_refresh: bool = refresh_apis == "all"
    ...
    if not views or refresh_apis == "all":
        views = {**views_special}
    ...
    for view_dir in view_dirs:
        if view_dir.startswith("{HOME}"):
            view_dir = f"{HOMEDIR}{view_dir[len('.HOME.'):]}"
        if view_dir.startswith(f"~{os.path.sep}"):
            view_dir = f"{HOMEDIR}{os.path.sep}{view_dir[2:]}"

        if not os.path.isdir(view_dir):
            continue

        for filename in natsorted(os.listdir(view_dir)):
            if filename == "__resource_type_index.yaml":
                continue
            ...
            if filename == "__event_reasons.yaml":
                ...
                for reason, data in d.items():
                    ...
                    if data and deep_get(data, DictPath("field_colors")):
                        ...
                        event_reasons[reason] = ThemeAttr(context, ftype)
                continue
            ...
            tmp = match_tmp[1]
            if "." in tmp:
                kind, api_family = tmp.split(".", maxsplit=1)
            else:
                kind = tmp
                api_family = ""
            # If we're only refreshing the selected view we skip all other elements
            if refresh_apis == "selected":
                if (kind, api_family) == deep_get(views, DictPath(f"{defaultview}#kind")):
                    views.pop(defaultview)
                    view_files.append(os.path.join(view_dir, filename))
            elif (kind, api_family) in available_api_families or kind.startswith("__"):
                view_files.append(os.path.join(view_dir, filename))
    ...
    for view_file in view_files:
        ...
        try:
            d = dict(secure_read_yaml(FilePath(view_file), directory_is_symlink=True))
                ...
                ruyaml.scanner.ScannerError,
                TypeError) as e:
            printable_errmsg: list[list[tuple[str, str]]] = [
                [("View-file ", "default"),
                 (f"{view_file}", "path"),
        ...
        if "infoview" in d:
            ...
            if infopad is not None:
                ...
                # In the default case this is "status".
                if "conditions_path" in infopad:
                    infoview_entry["conditions_path"] = \
                        deep_get(infopad, DictPath("conditions_path"))
            ...
            # Always include the shortcut for edit resource, unless overridden
            if "Owner References" not in _shortcuts:
                _shortcuts["Owner References"] = {
                    "key": "o",
                    "modifier": "shift",
                    "read_only": True,
...
def list_views(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    init_kubernetes_client()
    ansithemeprint([ANSIThemeStr("Checking available Kubernetes APIs", "default")])
    ansithemeprint([ANSIThemeStr("and updating list of supported views.\n", "default")])
    populate_views(refresh_apis="all")
    customise_listviews()
...
def checkforview(arg: str | tuple[str, str]) -> str | None:
    """
...
def open_view(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    for opt, optarg in options:
        ...
        elif opt == "--inject-action":
            defaultlistaction = optarg
        elif opt == "--format":
            overrideformat = optarg
        elif opt == "--inject-view":
            try:
                tmppath = ""
                if args:
                    tmppath = args[0]
                    ...
                    tmppath = os.getcwd()
                if tmppath.startswith(f"~{os.path.sep}"):
                    tmppath = f"{HOMEDIR}{os.path.sep}{tmppath[2:]}"
                if not tmppath.startswith(os.path.sep):
                    tmppath = f"{os.getcwd()}{os.path.sep}{tmppath}"
                defaultpath = (tmppath, os.path.basename(tmppath))
                args.insert(0, optarg)
            except IndexError:
                pass

    if defaultpath[0] and overrideformat:
        defaultpath = (defaultpath[0], overrideformat)
    ...
    if len(args) > 1:
        ...
        if initial_container is not None and defaultview not in ("Pods", "Config Maps", "Secrets"):
            ...
            ansithemeprint([ANSIThemeStr("Specifying a member", "description"),
                            ...
                            ANSIThemeStr("secret", "command"),
                            ANSIThemeStr(" views.", "default")], stderr=True)
...
COMMANDLINE: dict[str, CommandType] = {
    "View": {
        "command": ["view"],
        "description": [
            ANSIThemeStr("Open file viewer", "description")],
        ...
        "options": {
            "--format": {
                ...
                "description":
                    [ANSIThemeStr("File FORMAT", "description")],
                "extended_description": [
                    [ANSIThemeStr("If the file format cannot be deduced from",
                                  "description")],
                    [ANSIThemeStr("the name this option can be used to specify",
                                  "description")],
        ...
        "optional_args": [
            {
                ...
                "string": [
                    ANSIThemeStr("PATH", "argument")],
                "validation": {
                    "validator": "path_file_or_dir",
                },
            },
        ],
        "implicit_options": [
            ("--inject-action", "View File"),
            ("--inject-view", "debuglogs"),
        ],
        "callback": open_view,
    },
        ...
        "optional_args": [
            ...
            },
            {
                ...
                "validation": {
                    ...
                    # object (RFC-1123 dns-subdomain):configmap_data ([A-Za-z0-9_.-)
                    ...
                    # namespace (RFC-1123 dns-label)/name (RFC-1123 dns-subdomain):configmap_data
                    # ([A-Za-z0-9_.-])
                    #
                    # Note that containers will be accepted by the configmap_data regex,
                    # and thus does not need a separate regex. This means that invalid
                    # container names can be specified, but this isn't an issue since at worst
                    # cmu will simply fail to find them.
                    #
                    # Maybe we should modify the command parser to optionally split using
                    # a regex before checking arguments?
                    # we could do r"([^/]*)/?([^:]+):?(.*)" and validate each individual
                    # component separately. This wouldn't solve the Pod/ConfigMap/others
                    # issue, but it'd make things clearer. We could then use different
                    # validators for different components instead of having to use regex.
                    "regex":
                        r"^("
                        r"[a-z0-9][a-z0-9.-]{0,251}[a-z0-9]?:?|"
                        r"[a-z0-9][a-z0-9.-]{0,251}[a-z0-9]?:[a-z0-9][a-z0-9._-]{0,61}[a-z0-9]?|"
...
def main() -> None:
    ...
    CursesConfiguration.borders = deep_get(cmtlib.cmtconfig, DictPath("Global#borders"), True)
