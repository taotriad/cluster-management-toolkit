...
EXECUTABLE_SYMLINKS = [
    # These are Python scripts
    (FilePath(os.path.join(os.getcwd(), "cmtadm.py")), FilePath(os.path.join(BINDIR, "cmtadm"))),
    (FilePath(os.path.join(os.getcwd(), "cmtinv.py")), FilePath(os.path.join(BINDIR, "cmtinv"))),
    (FilePath(os.path.join(os.getcwd(), "cmt.py")), FilePath(os.path.join(BINDIR, "cmt"))),
    (FilePath(os.path.join(os.getcwd(), "cmu.py")), FilePath(os.path.join(BINDIR, "cmu"))),
]
...
def install_software_suse(packages: dict | set, verbose: bool = False) -> None:
    ...
    for pkg, data in packages.items():
        ...
        for line in result.stdout.decode("utf-8", errors="replace").splitlines():
            if (re_tmp := compiled_regex.match(line)) is None:
                continue
            pkg_version = re_tmp[1]
            break
        if pkg_version is not None:
            args = [zypper_path, "versioncmp", pkg_version, recommended_minimum]
            result = subprocess.run(args, stdout=PIPE, stderr=STDOUT, check=False)
            if "older" in result.stdout.decode("utf-8", errors="replace").splitlines():
...
def install_software_with_pip(packages: list[str],
    ...
    for path in (FilePath(f"pip3-{python_major}.{python_minor}"),
                 FilePath(f"pip3-{python_major}{python_minor}"),
                 FilePath(f"pip-{python_major}.{python_minor}"),
                 FilePath(f"pip-{python_major}{python_minor}"),
                 FilePath(f"pip{python_major}.{python_minor}"),
                 FilePath(f"pip{python_major}{python_minor}"),
                 FilePath("pip3")):
        try:
            # Depending on what distro we use we might need to use a versioned pip,
            # so test multiple paths until we find a suitable one.
            pip3_path = secure_which(path, fallback_allowlist=["/bin", "/usr/bin"],
                                     security_policy=SecurityPolicy.ALLOWLIST_RELAXED)
            break
        except FileNotFoundError:
            pass
...
def install(options: list[tuple[str, str]], args: list[str]) -> int:
    ...
    if distro_id_like and distro_id and distro_id == "rhel":
        distro = distro_id
    elif distro_id_like:
    ...
    version_id_tuple = [0, 0, 0]

    if version_id:
        try:
            for i, val in enumerate(version_id.split(".")):
                if i < 3:
                    version_id_tuple[i] = int(val)
        except ValueError:
            pass
    ...
    if distro == "suse" and version_id_tuple[0] < 15:
        ...
        ansithemeprint([ANSIThemeStr("depending on your SP-level this may be, for instance, “",
                        ...
                        ANSIThemeStr("“.\n", "default")])
    elif distro == "rhel" and version_id_tuple[0] < 9:
        ansithemeprint([ANSIThemeStr("Note:", "note"),
