...
from clustermanagementtoolkit.cmtlib import identify_arch, identify_distro, read_cmtconfig
...
from clustermanagementtoolkit.kubernetes_helper import get_node_roles, get_cluster_name
...
def get_control_plane_version(controlplane: str, k8s_distro: str) -> str:
    ...
    if k8s_distro == "rke2":
        ...
        for line in tmp:
            if (re_tmp := package_version_regex.match(line)) is None:
                continue
            package = re_tmp[1]
            package_version = re_tmp[2]
            if package == "kubeadm":
...
def rebuild_installation_info(state: str | None = None) -> None:
    """
...
def get_installation_info(cluster_name: str | None = None,
    ...
    try:
        info = dict(secure_read_yaml(CMT_INSTALLATION_INFO_FILE, checks=security_checks))
        ...
        if not ignore_non_existing:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ...
                            ANSIThemeStr(f"{CMT_INSTALLATION_INFO_FILE}", "path"),
                            ANSIThemeStr("is invalid; aborting.", "default")], stderr=True)
            sys.exit(errno.EINVAL)
...
def update_installation_info(**kwargs: Any) -> dict:
    ...
    """
    installation_target: str | None = deep_get(kwargs, DictPath("installation_target"))
    cluster_name: str | None = deep_get(kwargs, DictPath("cluster_name"))
    distro: str | None = deep_get(kwargs, DictPath("distro"))
    version: str | None = deep_get(kwargs, DictPath("version"))
    requested_version: str | None = deep_get(kwargs, DictPath("requested_version"))
    state: str | None = deep_get(kwargs, DictPath("state"))
    phase: int | str | None = deep_get(kwargs, DictPath("phase"))
    phase_skiplist: list[str] | None = deep_get(kwargs, DictPath("phase_skiplist"))
    cni: str | None = deep_get(kwargs, DictPath("cni"))
    pod_network_cidr: str | None = deep_get(kwargs, DictPath("pod_network_cidr"))
    cri: str | None = deep_get(kwargs, DictPath("cri"))
    control_planes: list[str] | None = deep_get(kwargs, DictPath("control_planes"))
    nodes: list[str] = deep_get(kwargs, DictPath("nodes"))
...
def check_and_print_status(retval: bool, ignore_errors: bool = False) -> None:
    """
    ...
    if retval:
        ...
        ansithemeprint([ANSIThemeStr("NOT OK", "notok"),
                        ANSIThemeStr("; ignoring errors.", "default")], stderr=True)
    else:
        ansithemeprint([ANSIThemeStr("NOT OK", "notok"),
                        ANSIThemeStr("; aborting.", "default")], stderr=True)
        sys.exit(errno.EINVAL)
...
def check_version_from_url(url: str, version_regex: str) -> list[str]:
    ...
    if url is not None:
        with tempfile.TemporaryDirectory() as td:
            check_and_print_status(download_files(td,
                                                  [(url, "version.txt", None, None)],
                                                  permissions=0o600))
            ...
            for line in versionoutput:
                if (re_tmp := _version_regex.match(line)) is not None:
                    version = list(re_tmp.groups())
                    break
    return version
...
def check_version_from_executable(command: FilePath, args: list[str],
    ...
    if cpath is not None:
        result, _retval = execute_command_with_response([cpath] + args)
...
def __upgrade_cni(cni: str, upgradetype: str, context: str,
    ...
    it can either upgrade the CNI itself or a helper executable.
        ...
        Parameters:
            ...
            **kwargs (dict[str, Any]): Keyword arguments
                ...
                verbose (bool): Be extra verbose
    """
    verbose = deep_get(kwargs, DictPath("verbose"), False)
    cni_version = deep_get(kwargs, DictPath("cni_version"))
    download_only = deep_get(kwargs, DictPath("download_only"), False)

    if upgradetype not in ("CNI", "executable"):
        raise ValueError(f"Unknown upgradetype {upgradetype}; this is a programming error.")

    arch_aliases = deep_get(cni_data, DictPath(f"{cni}#{upgradetype}#arch_aliases"), {})

    # Just to mess things up it's not enough to know the arch that the distro
    # expects; we sometimes need the arch that github expects.
    arch = identify_arch()
    arch = deep_get(arch_aliases, DictPath(arch), arch)
    ...
    if cni_version is not None:
        if (re_tmp := re.match(manual_candidate_version_regex, cni_version)) is None:
        ...
        candidate_version = list(re_tmp.groups())  # type: ignore
    ...
    if "urls" in deep_get(cni_data, DictPath(f"{cni}#{upgradetype}"), {}):
        ...
        if download_only or (version is None or not version) \
            ...
            for url in deep_get(cni_data, DictPath(f"{cni}#{upgradetype}#urls"), []):
                ...
                if upgradetype == "CNI":
                    ...
                    kubectl_major_version, kubectl_minor_version, *_ignore = \
                        kubernetes_helper.kubectl_get_version(client_only=True)
                    ...
                    if not download_only:
                        ...
                        if verbose:
                            ...
                            # This should check whether kubectl is >= 1.22;
                            # otherwise we need to use create
                            _result, _retval = execute_command_with_response(args)
        else:
    ...
    elif upgrade_command is not None and not download_only:
        if version is None or version < candidate_version:
            ...
            if verbose:
                print("  Upgrade Command: " + " ".join(substitute_list(upgrade_command,
                                                                       substitutions)))
...
def teardown_cni(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    # We now have an understanding of what CNI is expected to be installed.
    # We cannot rely on the CNI files still being downloaded in the expected location,
    # nor that any helpers will be installed, so start by downloading them.
    __upgrade_cni(cni=cni, upgradetype="executable", context=context_name, download_only=True,
                  pod_network_cidr=pod_network_cidr)
    __upgrade_cni(cni=cni, upgradetype="CNI", context=context_name, download_only=True,
                  pod_network_cidr=pod_network_cidr)
    ...
    for cni_filename in cni_filenames:
        ...
        if security_violations != [SecurityStatus.OK]:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(f": The CNI deployment file {cni_filename} is either "
    ...
    for cni_filename in cni_filenames:
        ...
        kubectl_path = secure_which(FilePath("/usr/bin/kubectl"),
                                    fallback_allowlist=["/etc/alternatives"],
                                    security_policy=SecurityPolicy.ALLOWLIST_RELAXED)
        _result, _retval = execute_command_with_response([kubectl_path, "delete", "-f", cni_path])
    ...
    playbooks = populate_playbooks_from_filenames(uninstall_cni_playbooks)
...
def upgrade_cni(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    pod_network_cidr: str | None = None
    verbose: bool = False
    ...
    for opt, optarg in options:
        if opt == "install":
            ...
            verbose = True
        elif opt == "--cni-version":
            cni_version = optarg
        elif opt == "--cni-executable-version":
            cni_executable_version = optarg
    ...
    __upgrade_cni(cni, "executable", context_name, pod_network_cidr,
                  cni_version=cni_executable_version, verbose=verbose)
    __upgrade_cni(cni, "CNI", context_name, pod_network_cidr,
                  cni_version=cni_version, verbose=verbose)
...
def __setup_cni(cni: str, context_name: str, cluster_name: str, **kwargs: Any) -> bool:
    ...
    Setup a CNI.
        ...
        Parameters:
            ...
            **kwargs (dict[str, Any]): Keyword arguments
                cni_executable_version (str): A hardcoded CNI executable version
                cni_version (str): A hardcoded CNI version
    ...
    verbose = deep_get(kwargs, DictPath("verbose"), False)
    cni_version = deep_get(kwargs, DictPath("cni_version"))
    cni_executable_version = deep_get(kwargs, DictPath("cni_executable_version"))
    ...
    upgrade_cni(options=[("install", ""),
                         ("--verbose", verbose),
                         ("--cni-version", cni_version),
                         ("--cni-executable-version", cni_executable_version)], args=pass_cni)
    # FIXME
    return True
...
def create_ssh_key() -> bool:
    """
    Create a new ssh key (ED25519 format).
    Note: This function should be modified to allow the user to default to an existing key
    even if that key isn't in ed25519 format.

        Returns:
            retval (bool): True on success, False on failure
    """
    if not os.path.exists(f"{HOMEDIR}/.ssh/id_ed25519"):
        args = [SSH_KEYGEN_BIN_PATH] + SSH_KEYGEN_ARGS + ["-f", f"{SSH_DIR}/id_ed25519"]
...
def add_ssh_keys_to_authorized_keys() -> bool:
    ...
    for path in Path(SSH_DIR).glob("*.pub"):
        ...
        try:
            if (tmp := secure_read_string(authorized_keys_path)):
                for line in tmp:
                    if line == pubkey[0]:
                        exists = True
                        break
...
def __task_check_and_create_ssh_key(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    and creates one if not.
        ...
        Parameters:
            ...
            **kwargs (dict[str, Any]): Keyword arguments
                confirm (bool): Ask for confirmation before creating an ssh key if none
                                exists; if confirm=False then this step will fail instead
    """
    confirm: bool = deep_get(kwargs, DictPath("confirm"), True)
...
# pylint: disable-next=unused-argument
def __task_scan_and_add_ssh_keys(installation_info: dict, **kwargs: Any) -> None:
    """
...
# pylint: disable-next=unused-argument
def __task_add_ssh_keys_to_inventory(installation_info: dict, **kwargs: Any) -> None:
    """
...
# pylint: disable-next=unused-argument
def __task_check_and_install_ansible_posix(installation_info: dict, **kwargs: Any) -> None:
    """
...
def check_versions(pkg_packages: list[str], version_checks: list[tuple[str, list[str], str]],
    ...
    if not (crc_upstream_version := get_latest_upstream_version("crc")):
        crc_upstream_version = "<unknown>"

    crc_version_regex = r"^CRC version: (\d+)(\.)(\d+)(\.)(.+)$"
    crc_installed_version = \
        fieldgetters.fieldgetter_executable_version(executable="crc",
                                                    args=["version"],
                                                    version_regex=crc_version_regex)[0]
    ...
    for version_check in version_checks:
        ...
        if regex is None:
            ...
            if (re_tmp := re.match(regex, response)) is not None:
                tmp_installed_version = re_tmp[1]
            else:
    ...
    for software, installed_version, candidate_version, _ in \
            cast(list[tuple[str, str, str, list[str]]], natsorted(pkg_versions)):
    ...
    print()
    for software, installed_version, candidate_version, _ in \
            cast(list[tuple[str, str, str, list[str]]], natsorted(other_versions)):
        iformat = __get_theme(installed_version, "version")
        cformat = __get_theme(candidate_version, "version")
        ansithemeprint([ANSIThemeStr(f"{software.ljust(slen + 2)}", "default"),
                        ANSIThemeStr(f"{installed_version.ljust(ilen + 2)}", iformat),
...
def run_playbooks(playbooks: list[tuple[list[ANSIThemeStr], FilePath]],
                  ...
                  extra_values: dict | None = None, **kwargs: Any) -> bool:
    """
...
def __playbook_paths_from_path(playbook_path: FilePath) -> list[FilePath]:
    ...
    # Note: cannot use glob, since glob neither supports
    # "{pattern1,pattern2} nor ("pattern1", "pattern2)
    for path in Path(playbook_path).iterdir():
        ...
        if path.suffix not in (".yml", ".yaml"):
            continue
        playbook_paths.append(FilePath(path))
...
def populate_playbooks_from_dir(path: FilePath) -> list[tuple[list[ANSIThemeStr], FilePath]]:
    ...
    return populate_playbooks_from_filenames(playbook_paths)


# pylint: disable-next=unused-argument
def __task_request_ansible_password(installation_info: dict, **kwargs: Any) -> None:
    """
...
def __run_playbooks_on_selection(playbooks: list[tuple[list[ANSIThemeStr], FilePath]],
                                 selection: list[str], extra_values: dict | None = None,
                                 **kwargs: Any) -> None:
    """
    A helper that runs a playbook list on a selection.
        ...
        Parameters:
            ...
            extra_values (dict): A dict of values to set before running the playbook
            **kwargs (dict[str, Any]): Keyword arguments
                ignore_errors (bool): Continue even if an error is encountered
    """
    ignore_errors: bool = deep_get(kwargs, DictPath("ignore_errors"), False)

    result = run_playbooks(playbooks, hosts=selection, extra_values=extra_values, **kwargs)
...
def __generic_task_run_playbooks_on_selection(selection: list[str], installation_info: dict,
    ...
    playbooks = populate_playbooks_from_filenames(paths)
    if extra_values is None:
        extra_values = {}
    __run_playbooks_on_selection(playbooks=playbooks, selection=selection,
                                 extra_values=extra_values, **kwargs)


def __task_run_prepreparation_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_PRE_PREPARE_DIR not in str(e):
            raise


# pylint: disable-next=unused-argument
def __task_run_preparation_playbooks_on_localhost(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    playbooks = populate_playbooks_from_filenames(prepare_targets["localhost"]["playbooks"])
    packages = []
    held_packages = []
...
# pylint: disable-next=too-many-locals
def __task_run_preparation_playbooks_on_hosts(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    playbooks = populate_playbooks_from_filenames(prepare_targets[k8s_distro]["playbooks"])
...
def __task_run_postpreparation_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_POST_PREPARE_DIR not in str(e):
            raise


# pylint: disable-next=unused-argument
def __task_setup_bash_completion(installation_info: dict, **kwargs: Any) -> None:
    """
    An installer task that sets up bash completion.
        ...
        Parameters:
            installation_info (dict): A dict with installation information [unused]
            **kwargs (dict[str, Any]): Keyword arguments
                verbose (bool): Be verbose when creating directories
    """
    verbose: bool = deep_get(kwargs, DictPath("verbose"), False)

    secure_mkdir(FilePath(HOMEDIR).joinpath(".local"), verbose=verbose)
    secure_mkdir(FilePath(HOMEDIR).joinpath(".local", "share"), verbose=verbose)
    secure_mkdir(BASH_COMPLETION_BASE_DIR, verbose=verbose)
    secure_mkdir(BASH_COMPLETION_DIR, verbose=verbose)
...
def __task_run_presetup_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_PRE_SETUP_DIR not in str(e):
            raise


def __task_run_setup_playbooks_on_localhost(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    playbooks = \
        populate_playbooks_from_filenames(setup_control_plane_targets["localhost"]["playbooks"])
...
def __task_run_setup_playbooks_on_control_planes(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    playbooks = \
        populate_playbooks_from_filenames(setup_control_plane_targets[k8s_distro]["playbooks"])
    ...
    if k8s_distro == "rke2":
        cri_socket = cri_socket.removeprefix("unix://")
...
def __task_run_postsetup_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_POST_SETUP_DIR not in str(e):
            raise


def merge_kubeconfigs(kubeconfig1: dict[str, Any] | ruyaml.comments.CommentedSeq,
                      kubeconfig2: dict[str, Any] | ruyaml.comments.CommentedSeq) \
        -> dict[str, Any] | ruyaml.comments.CommentedSeq:
    """
    Merge two kubeconfig dicts.

        Parameters:
            kubeconfig1 (dict[str, Any]): A dict with a kubeconfig (or an empty dict)
            kubeconfig2 (dict[str, Any]): A dict with a kubeconfig
        Returns:
            (dict[str, Any]):
    """
    merged_kubeconfig: dict[str, Any] = {}

    # We don't want any duplicates, so until we write the data we can store these
    # in dicts instead; this simplifies lookups.
    clusters: dict[str, Any] = {}
    contexts: dict[str, Any] = {}
    users: dict[str, Any] = {}

    if kubeconfig1:
        merged_kubeconfig = dict(kubeconfig1)
    else:
        merged_kubeconfig = dict(kubeconfig2)
        kubeconfig1["clusters"] = []
        kubeconfig1["contexts"] = []
        kubeconfig1["users"] = []

    for cluster in deep_get(kubeconfig1, DictPath("clusters"), []) \
            + deep_get(kubeconfig2, DictPath("clusters"), []):
        cluster_name = deep_get(cluster, DictPath("name"), "")
        if cluster_name in clusters:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ...
                            ANSIThemeStr("; manual merge is necessary. ", "default"),
                            ANSIThemeStr("Merge aborted.", "default")], stderr=True)
            return kubeconfig1
        clusters[cluster_name] = dict(cluster)

    for context in deep_get(kubeconfig1, DictPath("contexts"), []) \
            + deep_get(kubeconfig2, DictPath("contexts"), []):
        context_name = deep_get(context, DictPath("name"), "")
        if context_name in contexts:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": A context named ", "default"),
                            ANSIThemeStr(context_name, "argument"),
                            ANSIThemeStr(" already exists in ", "default"),
                            ANSIThemeStr(KUBE_CONFIG_FILE, "path"),
                            ANSIThemeStr("; manual merge is necessary. ", "default"),
                            ANSIThemeStr("Merge aborted.", "default")], stderr=True)
            return kubeconfig1
        contexts[context_name] = dict(context)

    for user in deep_get(kubeconfig1, DictPath("users"), []) \
            + deep_get(kubeconfig2, DictPath("users"), []):
        user_name = deep_get(user, DictPath("name"), "")
        if user_name in users:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": A user named ", "default"),
                            ANSIThemeStr(user_name, "argument"),
                            ANSIThemeStr(" already exists in ", "default"),
                            ANSIThemeStr(KUBE_CONFIG_FILE, "path"),
                            ANSIThemeStr("; manual merge is necessary. ", "default"),
                            ANSIThemeStr("Merge aborted.", "default")], stderr=True)
            return kubeconfig1
        users[user_name] = dict(user)

    # Now that we've merged the values we update the merged_config
    merged_kubeconfig["clusters"] = list(clusters.values())
    merged_kubeconfig["contexts"] = list(contexts.values())
    merged_kubeconfig["users"] = list(users.values())

    return merged_kubeconfig


# pylint: disable-next=unused-argument
def __task_import_kubeconfig(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    d1: dict[str, Any] | ruyaml.comments.CommentedSeq = {}

    # This read must not fail, so no exceptions
    d2: dict[str, Any] | ruyaml.comments.CommentedSeq = secure_read_yaml(config_file_name)
    ...
    # Before merging we may need to rename the cluster, the admin, and the context
    for cluster in deep_get(d2, DictPath("clusters"), []):
        if deep_get(cluster, DictPath("name")) in (cluster_name, "default"):
            cluster["name"] = cluster_name

    for context in deep_get(d2, DictPath("contexts"), []):
        if deep_get(context, DictPath("name")) in (context_name, "default",
                                                   f"kubernetes-admin@{cluster_name}"):
            context["context"]["cluster"] = cluster_name
            context["context"]["user"] = admin_name
            context["name"] = context_name

    for user in deep_get(d2, DictPath("users"), []):
        if deep_get(user, DictPath("name")) in (admin_name, "default", "kubernetes-admin"):
            user["name"] = admin_name

    if deep_get(d2, DictPath("current-context")) in (context_name, "default",
                                                     f"kubernetes-admin@{cluster_name}"):
        d2["current-context"] = context_name

    d1 = merge_kubeconfigs(d1, d2)
...
def __task_adjust_kubeconfig_server_address(installation_info: dict, **kwargs: Any) -> None:
    """
...
def __task_setup_cni(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    __setup_cni(cni, context_name, cluster_name, verbose=verbose)


# pylint: disable-next=unused-argument
def __task_drain_control_planes(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    args = ["/usr/bin/kubectl", "drain",
            "--ignore-daemonsets", "--delete-emptydir-data", "--disable-eviction"]

    for controlplane in controlplanes:
        check_and_print_status(execute_command(args + [controlplane]), ignore_errors=ignore_errors)


# pylint: disable-next=unused-argument
def __task_uncordon_control_planes(installation_info: dict, **kwargs: Any) -> None:
    """
...
def __task_run_preupgrade_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_PRE_UPGRADE_DIR not in str(e):
            raise


# Run upgrade playbooks on control planes
def __task_run_upgrade_playbooks_on_control_planes(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    selection = __selection_control_planes()
    playbooks = \
        populate_playbooks_from_filenames(upgrade_control_plane_targets[k8s_distro]["playbooks"])
    cri = installation_info[cluster_name]["cri"]
    if cri not in cri_data:
        ansithemeprint([ANSIThemeStr("Error", "error"),
                        ANSIThemeStr(": CRI undefined or unknown (value: ", "default"),
                        ANSIThemeStr(f"{cri}", "argument"),
                        ANSIThemeStr("); aborting.", "default")], stderr=True)
        sys.exit(errno.EINVAL)
    cri_socket = deep_get(cri_data[cri], DictPath("socket"))
...
def __task_run_postupgrade_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_POST_UPGRADE_DIR not in str(e):
            raise


# pylint: disable-next=unused-argument
def __task_verify_that_cluster_has_no_nodes(installation_info: dict, **kwargs: Any) -> None:
    """
    A task that verifies that no non-control plane nodes remain in the cluster.
        ...
        Parameters:
            ...
            **kwargs (dict[str, Any]): Keyword arguments [unused]
    """
    node_statuses = __get_node_info(**kwargs)
...
def __task_run_preteardown_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_PRE_TEARDOWN_DIR not in str(e):
            raise


# Run teardown playbooks on control planes
def __task_run_teardown_playbooks_on_control_planes(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    selection = __selection_control_planes()
    playbooks = \
        populate_playbooks_from_filenames(teardown_control_plane_targets[k8s_distro]["playbooks"])

    __run_playbooks_on_selection(playbooks=playbooks, selection=selection,
                                 extra_values={}, **kwargs)


def __task_run_postteardown_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_POST_TEARDOWN_DIR not in str(e):
            raise


# pylint: disable-next=unused-argument,too-many-branches
def __task_remove_kubeconfig(installation_info: dict, **kwargs: Any) -> None:
    """
...
def __task_run_prepurge_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_PRE_PURGE_DIR not in str(e):
            raise


# Run purge playbooks on control planes
def __task_run_purge_playbooks_on_control_planes(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    held_packages = []
    playbooks = \
        populate_playbooks_from_filenames(purge_control_plane_targets[k8s_distro]["playbooks"])
...
def __task_run_postpurge_playbooks(installation_info: dict, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if CMT_POST_PURGE_DIR not in str(e):
            raise
...
def __list_phases(phases: list[tuple[list[ANSIThemeStr], Callable[..., None]]],
                  start_phase: int | None = 0, phase_skiplist: list | None = None) -> None:
    """
...
def run_tasks(tasks: list[tuple[list[ANSIThemeStr], Callable[..., None]]],
              ...
              final_state: str, **kwargs: Any) -> None:
    """
    Run tasks.

        Parameters:
            tasks [([themestring], Callable)]:
                ([themestring]): A description of the task in the form of list[ANSIThemeStr]
                (Callable): A function areference
            phase (int): The installation phase
            phase_skiplist (set(int)): A set of phases to skip
    ...
    for i in range(phase, len(tasks)):
        ...
        if i not in set(phase_skiplist):
            tasks[i][1](installation_info=installation_info,
...
# pylint: disable-next=too-many-branches
def __find_requested_version(k8s_distro: str, version: str | None = None) -> str:
    """
    ...
    """
    os_distro = identify_distro()
    versions: list[tuple[str, str, str, list[str]]] = []
...
def import_cluster(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    __task_check_and_create_ssh_key(installation_info={}, verbose=verbose)
    __task_add_ssh_keys_to_inventory(installation_info={}, verbose=verbose)
    print()
    ...
    for cluster_name, context in clusters:
        ...
        if not kh.set_context(name=context, unchanged_is_success=True):
            ...
            failures.append(cluster_name)
            continue

        vlist, status = kh.get_list_by_kind_namespace(("Node", ""), "")
        if status != 200:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": The API-server for cluster ", "default"),
                            ANSIThemeStr(f"{cluster_name}", "hostname"),
        ...
        if vlist is None:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": The API-server for cluster ", "default"),
                            ANSIThemeStr(f"{cluster_name}", "hostname"),
                            ANSIThemeStr(" did not return any data", "default"),
                            ANSIThemeStr("; skipping.", "default")], stderr=True)
            failures.append(cluster_name)
            continue
        ...
        if controlplane is None:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": Could not find a control plane", "default"),
                            ANSIThemeStr(" for cluster ", "default"),
                            ANSIThemeStr(f"{cluster_name}", "hostname"),
                            ANSIThemeStr("; skipping.", "default")], stderr=True)
            failures.append(cluster_name)
        ...
        retval, ansible_results = \
            run_playbook(get_playbook_path(FilePath("prepare_passwordless_ansible.yaml")),
                         hosts=[controlplane], extra_values=extra_values, verbose=verbose)

        if not ansible_results:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": The Ansible playbook “", "default"),
                            ANSIThemeStr("prepare_passwordless_ansible.yaml", "path"),
                            ANSIThemeStr("“ failed on ", "default"),
                            ANSIThemeStr(controlplane, "hostname"),
        ...
        if not ansible_results:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": The Ansible playbook “", "default"),
                            ANSIThemeStr("get_versions.yaml", "path"),
                            ANSIThemeStr("“ failed on ", "default"),
                            ANSIThemeStr(controlplane, "hostname"),
                            ANSIThemeStr(" (retval: ", "default"),
                            ANSIThemeStr(f"{retval}", "errorvalue"),
        ...
        if not tmp:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": Received empty version data from ", "default"),
                            ANSIThemeStr(controlplane, "hostname"),
                            ANSIThemeStr("; skipping.", "default")], stderr=True)
            failures.append(cluster_name)
            continue
        ...
        for line in tmp:
            ...
            if package == "kubeadm":
                ...
                update_installation_info(cluster_name=cluster_name, distro=k8s_distro,
                                         ...
                                         pod_network_cidr=pod_network_cidr,
                                         control_planes=controlplanes, nodes=nodes, cri=cri)
                break
    ...
    if not failures:
        ...
        ansithemeprint([ANSIThemeStr("\nFailed to import the following clusters:", "warning")],
                       stderr=True)

        for cluster_name in failures:
            ansithemeprint([ANSIThemeStr("• ", "separator"),
                            ANSIThemeStr(f"{cluster_name}", "hostname")], stderr=True)


# pylint: disable-next=unused-argument
def import_kubeconfig(options: list[tuple[str, str]], args: list[str]) -> None:
    """
    Merge existing kubeconfig files with ~/.kube/config.

        Parameters:
            options ([(str, str)]): Options to use when executing this action
            args ([str]): Arguments to use when executing this action
    """
    kubeconfig: dict[str, Any] | ruyaml.comments.CommentedSeq = {}

    # First read .kube/config (if it exists)
    try:
        ...
        ansithemeprint([ANSIThemeStr("Note", "note"),
                        ...
                        ANSIThemeStr("a new file will be created.", "default")])
    ...
    for path in args[0].split(","):
        ...
        try:
            d = secure_read_yaml(FilePath(path))
        except FileNotFoundError as e:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": Failed to import ", "default"),
                            ANSIThemeStr(f"{path}", "path"),
                            ANSIThemeStr("; ", "default"),
                            ANSIThemeStr(f"{e}", "errorvalue"),
                            ANSIThemeStr(". Skipping.", "default")], stderr=True)
            continue
        kubeconfig = merge_kubeconfigs(kubeconfig, d)
        secure_write_yaml(KUBE_CONFIG_FILE, kubeconfig, permissions=0o600, sort_keys=False)
...
def check_for_updates(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    if any((update_pkg_cache, update_upstream_cache)):
    ...
    if update_upstream_cache:
        ansithemeprint([ANSIThemeStr("\n• ", "separator"),
                        ANSIThemeStr("Updating upstream cache (GitHub, etc.)", "action")])
        update_version_cache(verbose=verbose, force=force)

    ansithemeprint([ANSIThemeStr("\n[Summarising version data]", "phase")])

    pkg_packages: list[str] = [
        "ansible",
        "ansible-core",
        "containerd",
        "containerd.io",
        ...
        try:
            ...
            if str(e) in ("[Errno -2] Name or service not known",
                ...
                if (re_tmp := re.match(r"^\[Errno (-\d+)\] (.+)", str(e))) is None:
                    ...
                    # how to handle it
                    raise
                if not deep_get(failures, DictPath(re_tmp[2]), []):
                    failures[re_tmp[2]] = []
                failures[re_tmp[2]].append(controlplane)
...
def add_nodes(options: list[tuple[str, Any]], args: list[Any]) -> None:
    ...
    configuration_paths: list[FilePath] = []

    if k8s_distro == "kubeadm":
        configuration_paths = [
            FilePath("templates").joinpath("config", "join_configuration.yaml.j2"),
        ]
    elif k8s_distro == "rke2":
        configuration_paths = [
            FilePath("templates").joinpath("etc", "rancher", "rke2", "config.agent.yaml.j2"),
        ]
    ...
    crio_major_version = None
    crio_minor_version = None

    add_playbooks: list[FilePath] = []
    ...
    if cri == "containerd":
        ...
        if cri_version is None:
            tmp_crio_version = get_crio_version((int(requested_major), int(requested_minor)))
            if tmp_crio_version is None:
                check_and_print_status(False)
            # The type ignore below is necessary because mypy doesn't recognise that
            # check_and_print_status always aborts with sys.exit() when called with False
            crio_major_version, crio_minor_version = tmp_crio_version  # type: ignore
        else:
            # This case shouldn't be necessary; mypy should recognise that we're doing
            # a None-check first.
            tmp_crio_version = cast(tuple, cri_version.split("."))
            if len(tmp_crio_version) < 2:
                check_and_print_status(False)
            crio_major_version = tmp_crio_version[0]
            crio_minor_version = tmp_crio_version[1]
    ...
    minor_versions = []
    for minor_version in range(32, int(requested_minor) + 1):
        minor_versions.append(f"{minor_version}")
...
def create_cluster(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    if cd_api_version not in ("v1alpha1", "v1alpha2", "v1alpha3", "v1alpha4", "v1alpha5"):
        ansithemeprint([ANSIThemeStr("Error", "error"),
                        ANSIThemeStr(": ", "default"),
    ...
    cni_version = deep_get(cd, DictPath("options#cni_version"))
    if cni_version == "latest":
        cni_version = None
    cni_executable_version = deep_get(cd, DictPath("options#cni_executable_version"))
    if cni_executable_version == "latest":
        cni_executable_version = None
    ...
    suffix_admin = deep_get(cd, DictPath("options#suffixAdmin"), True)
    ...
    cri_version = deep_get(cd, DictPath("cluster#cri_version"))
    if cri_version == "auto":
        cri_version = None
    ...
    # Create the admin name and context name
    if suffix_admin:
        admin_name: str = f"kubernetes-admin+{cluster_name}"
    else:
        admin_name = "kubernetes-admin"
    context_name: str = f"{admin_name}@{cluster_name}"
    ...
    elif state not in ("adding_nodes", "installing", "installed", "deploying",
                       "<done>", "setup_cni", "deploy_workloads", "deployed", "prepared",
                       "upgraded"):
        ansithemeprint([ANSIThemeStr("\nError", "error"),
                        ANSIThemeStr(": Invalid installation state ", "default"),
                        ANSIThemeStr(f"{state}", "phase"),
    ...
    if state == "adding_nodes":
        ...
        node_global_cri = deep_get(cd, DictPath("nodes#options#cri"), cri)
        node_global_cri_version = deep_get(cd, DictPath("nodes#options#cri_version"), cri_version)
        if node_global_cri_version == "auto":
            node_global_cri_version = None
        ...
        for group, data in deep_get(cd, DictPath("nodes#groups"), {}).items():
            ...
            vcpus: int = deep_get(data, DictPath("vCPUs"), 4)
            extra_values["vcpus"] = vcpus
            vmram: int = deep_get(data, DictPath("vmRAM"), 4096)
            extra_values["vmram"] = vmram
            template_balloon_size: str = \
                FilePath(deep_get(data, DictPath("templateBalloonSize"), "5G"))
            template_name: str = deep_get(data, DictPath("vmTemplateImageName"), "template")
            template_mac: str = ""
            template_ip: str = ""
            nodes_tuple: list[tuple[str, str, str]] = []
            if tmp_nodes:
                ...
                for address_range in ranges:
                    ...
                    if isinstance(tmp_highrange, int):
                        ...
                        highrange = list(range(tmp_highrange[0], tmp_highrange[1] + 1))
                    if isinstance(tmp_lowrange, int):
                        lowrange = [tmp_lowrange]
                    else:
                        if len(tmp_lowrange) != 2:
                            sys.exit("An address-range must be [start, end]")
                        lowrange = list(range(tmp_lowrange[0], tmp_lowrange[1] + 1))
                    for highdigit in highrange:
                        for lowdigit in lowrange:
                            ...
                            nodename = substitute_string(hostname_template, {
                                                         "<<<item.1>>>": str(lowdigit),
                                                         "<<<item.0>>>": str(highdigit)})
                            nodes_tuple.append((nodename, ipaddress, macaddress))
                            hosts.append(nodename)

            node_cri = deep_get(cd, DictPath("options#cri"), node_global_cri)
            node_cri_version = deep_get(cd, DictPath("options#cri_version"),
                                        node_global_cri_version)
            if node_cri_version == "auto":
                node_cri_version = None
            ...
            if virtualise:
                retval = prepare_vm_template(vmhost,
                                             ...
                                             kubernetes_helper=kh, cri=cri, verbose=verbose)
                if retval:
                    ansithemeprint([ANSIThemeStr("Error", "error"),
                                    ANSIThemeStr(": failed to prepare a VM image template"
                                                 "; aborting.", "default")], stderr=True)
                    sys.exit(retval)
                retval = create_vm_hosts(vmhost, nodes_tuple,
                                         os_image=os_image,
                                         os_variant=os_variant,
                                         extra_values=extra_values, verbose=verbose)
                if retval:
    ...
    if state == "setup_cni":
        __setup_cni(cni, context_name, cluster_name,
                    cni_version=cni_version, cni_executable_version=cni_executable_version,
                    verbose=verbose)
        phase = 0
                ...
                if "git" in data:
                    ...
                    for uri in uris:
                        ...
                        if verbose:
                            ...
                            _result, _retval = execute_command_with_response(args + [uri], env=env)
                    if wtype in ("namespace", "secret+tls"):
                        dry_run_result, _retval = \
                    ...
                    if data is not None:
                        ...
                        if verbose:
                            ...
                            _result, _retval = execute_command_with_response(args + [path], env=env)
                elif "helm" in data:
                    ...
                    version = deep_get(deployment, DictPath("version"))
                    extra_flags = deep_get(deployment, DictPath("extraArgs"), [])
                    ...
                    # If redeploying workloads we might need to update the repo
                    args = [helm_path, "repo", "update"]
                    ...
                    args = [helm_path, "upgrade", hname, chart, "--namespace", namespace,
                            "--version", version, "--install"]
                    if create_namespace:
                        args.append("--create-namespace")

                    # Add extra flags if there are any
                    for flags in extra_flags:
                        if isinstance(flags, list):
                            ...
                            ansithemeprint([ANSIThemeStr("Error", "error"),
                                            ...
                                            ANSIThemeStr(f"{name}", "argument"),
                                            ANSIThemeStr(" is invalid; ", "default"),
                                            ANSIThemeStr("extraArgs", "emphasis"),
                                            ANSIThemeStr(" must be [str|list[str]]", "default"),
                                            ANSIThemeStr("; aborting.", "default")], stderr=True)
                            sys.exit(errno.EINVAL)
...
def setup_control_planes(options: list[tuple[str, Any]], args: list[str]) -> None:
    ...
    if k8s_distro == "kubeadm":
        setup_control_plane_tasks = __setup_kubeadm_control_plane_tasks
        configuration_paths = [
            FilePath("templates").joinpath("config", "cluster_configuration.yaml.j2"),
            FilePath("templates").joinpath("config", "init_configuration.yaml.j2"),
            FilePath("templates").joinpath("config", "kubelet_configuration.yaml.j2"),
            FilePath("templates").joinpath("config", "kube_proxy_configuration.yaml.j2"),
        ]
    elif k8s_distro == "rke2":
    ...
    if k8s_distro == "kubeadm":
        ...
        major_minor_patchrev = requested_version.split("-")
        # Now split the version tuple
        split_tuple = major_minor_patchrev[0].split(".")
        if len(split_tuple) < 2:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": Could not parse version “", "default"),
                            ANSIThemeStr(f"{major_minor_patchrev[0]}", "version"),
                            ANSIThemeStr("“; aborting.",
                                         "default")], stderr=True)
            sys.exit(errno.EINVAL)
        requested_major = split_tuple[0]
        requested_minor = split_tuple[1]

        cluster_configuration_api_version = "kubeadm.k8s.io/v1beta4"
        init_configuration_api_version = "kubeadm.k8s.io/v1beta4"
    ...
    if cri == "containerd":
        cri_playbook_path = FilePath("setup_containerd.yaml")
        setup_control_plane_targets[k8s_distro]["playbooks"].insert(0, cri_playbook_path)
    elif cri == "cri-o":
        cri_playbook_path = FilePath("setup_cri-o.yaml")
        setup_control_plane_targets[k8s_distro]["playbooks"].insert(0, cri_playbook_path)
        if cri_version is None:
            tmp_crio_version = get_crio_version((int(requested_major), int(requested_minor)))
            if tmp_crio_version is None:
    ...
    if k8s_distro == "rke2":
        ansithemeprint([ANSIThemeStr("\nNote", "note"),
                        ANSIThemeStr(": If this is an RKE2 installation you may need "
...
def __get_node_info(**kwargs: Any) -> list[dict]:
    ignore_errors: bool = deep_get(kwargs, DictPath("ignore_errors"), False)
    ...
    if not Path(KUBE_CONFIG_FILE).is_file():
        if ignore_errors:
            ansithemeprint([ANSIThemeStr("Warning", "warning"),
                            ...
                            ANSIThemeStr(f"{KUBE_CONFIG_FILE}", "path"),
                            ANSIThemeStr("; ignoring since “", "default"),
                            ANSIThemeStr("--ignore-errors", "option"),
    ...
    if kh is None:
        if ignore_errors:
            ansithemeprint([ANSIThemeStr("Warning", "warning"),
                            ANSIThemeStr(": Failed to initialise connection to ",
                                         "default"),
                            ANSIThemeStr("API-server; ignoring since “", "default"),
                            ANSIThemeStr("--ignore-errors", "option"),
                            ANSIThemeStr("“ was specified.", "default")], stderr=True)
            return []
        ansithemeprint([ANSIThemeStr("Critical", "critical"),
...
def get_control_planes(fail_on_empty: bool = True) -> list[tuple[str, str]]:
    ...
    for controlplane in controlplanes_:
        ...
        try:
            ...
            if str(e) == "[Errno -5] No address associated with hostname":
                ansithemeprint([ANSIThemeStr("Error", "error"),
                                ANSIThemeStr(": No address associated with hostname “", "default"),
                                ANSIThemeStr(f"{controlplane}", "hostname"),
                                ANSIThemeStr("“. Aborting.", "default")], stderr=True)
                sys.exit(errno.ENOENT)
...
def teardown_control_plane(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    installation_info = update_installation_info(state="torn_down", phase=0, cni="<none>")
...
def upgrade_control_plane(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    for opt, optarg in options:
        if opt == "--start-at-task":
            ...
            ansible_configuration["save_logs"] = True
        elif opt == "--ignore-feature-gates":
            ignore_feature_gates = True
        elif opt == "-Y":
            confirm = False
    ...
    if kh is None:
        kh = kubernetes_helper.KubernetesHelper(about.PROGRAM_SUITE_NAME,
                                                about.PROGRAM_SUITE_VERSION, None)
    if kh is not None:
        fg = kh.check_for_feature_gates()

        has_fg = []

        for config, feature_gates in fg.items():
            if feature_gates is not None and feature_gates:
                has_fg.append(config)
        if has_fg:
            ansithemeprint([ANSIThemeStr("Error", "error"),
                            ANSIThemeStr(": the following configurations have ", "default"),
                            ANSIThemeStr("changes to feature gates:", "default")], stderr=True)
            for key in has_fg:
                ansithemeprint([ANSIThemeStr("• ", "separator"),
                                ANSIThemeStr(f"{key}", "default")], stderr=True)
            ansithemeprint([ANSIThemeStr("\nUpgrading is not supported when", "default"),
                            ANSIThemeStr(" feature gates have been altered.", "default")],
                           stderr=True)

            if not ignore_feature_gates:
                ansithemeprint([ANSIThemeStr("To override, pass “", "default"),
                                ANSIThemeStr("--ignore-feature-gates", "option"),
                                ANSIThemeStr("“. Aborting.", "default")], stderr=True)
                sys.exit(errno.ENOTSUP)
            ansithemeprint([ANSIThemeStr("Warning", "warning"),
                            ANSIThemeStr(": “", "default"),
                            ANSIThemeStr("--ignore-feature-gates", "option"),
                            ANSIThemeStr("“ passed; continuing.", "default")], stderr=True)
    ...
    if update_cache:
        ...
        _retval, _ansible_results = \
            run_playbook(get_playbook_path(FilePath("add_kubernetes_repo.yaml")),
    ...
    pkg_versions: list[tuple[str, str, str, list[str]]] = []
    ...
    if k8s_distro == "kubeadm":
        ...
        if int(requested_minor) < 31:
            upgrade_configuration_api_version = "<unused>"
        else:
            upgrade_configuration_api_version = "kubeadm.k8s.io/v1beta4"

        upgrade_control_plane_targets[k8s_distro]["extra_values"] = {
            "upgrade_configuration_api_version": upgrade_configuration_api_version,
            # Add major/minor to extra_values
            "kubernetes_major_minor_version": f"{requested_major}.{requested_minor}",
        }
...
security_audit_checks: list[dict[str, Any]] = [
    {
...
def audit(options: list[tuple[str, str]], args: list[str]) -> None:
    ...
    """
    usergroup: str = ""
    usergroup_autodetect: bool = True
    ...
    if len(usergroup) == 0 and usergroup_autodetect:
        ...
        try:
            ...
            for line in logindefs.splitlines():
                if usergroups_regex.match(line) is not None:
                    usergroup = groupname
        except FilePathAuditError:
            # If there's no such file we default to using the group
            usergroup = groupname
...
troubleshoot_checks: list[dict[str, Any]] = [
...
COMMANDLINE: dict[str, CommandType] = {
    "Check Versions": {
        ...
        "options": {
            "--force": {
                "description": [
                    ANSIThemeStr("Force update of the version cache", "description")],
                "extended_description": [
                    [ANSIThemeStr("Typically version cache updates are", "description")],
                    [ANSIThemeStr("rate-limited to once per hour; this option", "description")],
                    [ANSIThemeStr("forces an update during the cooldown period.", "description")],
                ],
            },
            "--no-cache-update": {
                "description": [
                    ANSIThemeStr("Do not update the package caches", "description")],
                "extended_description": [
                    [ANSIThemeStr("Alias for:", "description")],
                    [ANSIThemeStr("“", "description"),
                     ANSIThemeStr("--no-pkg-cache-update", "option")],
                    [ANSIThemeStr(" --no-upstream-cache-update", "option"),
                     ANSIThemeStr("“.", "description")],
                ],
            },
            "--no-pkg-cache-update": {
                "description": [
                    ANSIThemeStr("Do not update the distro package cache", "description")],
            },
            "--no-upstream-cache-update": {
                "description": [
                    ANSIThemeStr("Do not update the upstream package cache", "description")],
            },
            "--verbose": {
                "description": [
    ...
    "Import kubeconfig": {
        ...
        "values": [
            ...
            ANSIThemeStr("]", "separator")],
        "description": [
            ANSIThemeStr("Merge kubeconfig(s) into ", "description"),
            ANSIThemeStr(f"{KUBE_CONFIG_FILE}", "path")],
        "options": {
        },
        "required_args": [
    ...
    "Setup Control Plane": {
        ...
        "extended_description": [
            ...
            [ANSIThemeStr("Warning: ", "warning"),
             ANSIThemeStr("weave", "argument"),
             ANSIThemeStr(" is no longer maintained upstream", "description")],
            [ANSIThemeStr("and is thus not recommended for production use.", "description")],
        ],
        "options": {
            "--resume": {
                "description": [
    ...
    "Upgrade Control Plane": {
        ...
        "options": {
            "--ignore-feature-gates": {
                "description": [
                    ANSIThemeStr("Ignore the result of the feature gates check", "description")],
                "extended_description": [
                    [ANSIThemeStr("Upgrading a cluster may fail if the default", "description")],
                    [ANSIThemeStr("set of feature gates has been altered; ", "description"),
                     ANSIThemeStr("by default", "description")],
                    [ANSIThemeStr("upgrades are aborted if such changes", "description"),
                     ANSIThemeStr("are detected.", "description")],
                    [ANSIThemeStr("This option makes the check non-aborting.", "description")],
                    [ANSIThemeStr("Note", "note"),
                     ANSIThemeStr(": this may yield a failed installation", "description")],
                    [ANSIThemeStr("or non-operational cluster", "description")],
                ],
            },
            "--no-cache-update": {
                "description": [
    ...
    "Teardown Control Plane": {
        ...
        "options": {
            "--ignore-errors": {
                "description": [
                    ANSIThemeStr("Ignore errors during teardown", "description")],
                "extended_description": [
                    [ANSIThemeStr("DANGEROUS", "critical"),
                     ANSIThemeStr(": Unconditionally tear down the cluster", "description")],
                    [ANSIThemeStr("even if the control plane isn't responding.", "description")],
                    [ANSIThemeStr("This may have unintended consequences,", "description")],
                    [ANSIThemeStr("such as leaving configuration files behind", "description")],
                ],
            },
            "--resume": {
                "description": [
